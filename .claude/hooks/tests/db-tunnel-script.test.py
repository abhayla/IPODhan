...
Class under test: `stop` must NEVER kill a process it did not record as ITS tunnel. Cases:
  ...
  - ... and a third waiter's empty lock dir appeared before put-back    -> never clobbered
  - the reclaim test hook without DB_TUNNEL_TEST_MODE=1                 -> never run
  - start's lock replaced by another process's while start runs        -> release leaves it
...
if [ "${DB_TUNNEL_TEST_FAKE_SSH_MODE:-exit}" = steal_lock ]; then
  # While start holds its lock, another process's lock replaces it (pid 424242).
  printf '424242' > "$DB_TUNNEL_LOCK_DIR/pid"
fi
echo 'fake ssh: no network in tests' >&2
exit 255
"""

# The reclaim-race seam (DB_TUNNEL_TEST_RECLAIM_HOOK, called with the phase as $1). Plan "fresh":
# a second waiter reclaimed the stale lock first and holds a FRESH one (pid 424242). Plan
# "fresh+third": additionally a third waiter has just made an EMPTY lock dir before put-back.
RECLAIM_HOOK = r"""#!/usr/bin/env bash
d="$DB_TUNNEL_LOCK_DIR"
printf '%s\n' "$1" >> "$DB_TUNNEL_TEST_HOOK_LOG"
case "$1:${DB_TUNNEL_TEST_HOOK_PLAN:-none}" in
  before-rename:fresh|before-rename:fresh+third)
    rm -f "$d/created_at" "$d/pid"; rmdir "$d"; mkdir "$d"
    date +%s > "$d/created_at"; printf '424242' > "$d/pid" ;;
  before-putback:fresh+third)
    mkdir "$d" ;;
esac
exit 0
"""
...
PS_LINE_SPACED_SSH_EXE = "%s   1   1   %s  ?   0   00:00 C:\\Program Files\\x\\my ssh.exe"
...
class DbTunnelScriptTest(unittest.TestCase):
    def setUp(self):
        ...
        # Forward slashes: the script runs the hook as one quoted path, which bash on Windows
        # resolves as C:/... but not with backslashes.
        self.reclaim_hook = os.path.join(self.tmp, "reclaim_hook").replace("\\", "/")
        with open(self.reclaim_hook, "w", encoding="utf-8", newline="\n") as f:
            f.write(RECLAIM_HOOK)
        os.chmod(self.reclaim_hook, 0o755)
        self.hook_log = os.path.join(self.tmp, "hook.log")
    ...
    def _base_env(self):
        ...
        env["DB_TUNNEL_TEST_MODE"] = "1"
        env["DB_TUNNEL_TEST_HOOK_LOG"] = self.hook_log
    ...
    def _hook_phases(self):
        if not os.path.exists(self.hook_log):
            return []
        with open(self.hook_log, "r", encoding="utf-8") as f:
            return [l for l in f.read().splitlines() if l]
    ...
    def test_second_reclaimer_never_deletes_a_fresh_lock(self):
        ...
        proc = self._run(
            ...
            env_overrides={
                ...
                "DB_TUNNEL_TEST_RECLAIM_HOOK": self.reclaim_hook,
                "DB_TUNNEL_TEST_HOOK_PLAN": "fresh",
            },
        )
        self.assertEqual(proc.returncode, 2, proc.stderr)
        self.assertEqual(self._hook_phases()[:1], ["before-rename"], proc.stderr)
        self.assertEqual(self._ssh_calls(), [])
    ...
    def test_putback_never_clobbers_a_third_waiters_lock(self):
        # As above, but between the rename and the put-back a THIRD waiter has made an empty
        # lock dir. The put-back must not overwrite it (two holders); it must leave the fresh
        # lock under its reclaim name and say so.
        self._write_lock(time.time() - 600, pid=111111)
        self._set_netstat([])
        proc = self._run(
            "start",
            env_overrides={
                ...
                "DB_TUNNEL_LOCK_WAIT_SECS": "1",
                "DB_TUNNEL_TEST_RECLAIM_HOOK": self.reclaim_hook,
                "DB_TUNNEL_TEST_HOOK_PLAN": "fresh+third",
            },
        )
        self.assertEqual(proc.returncode, 2, proc.stderr)
        self.assertEqual(self._ssh_calls(), [])
        self.assertIn("could not put it back", proc.stderr)
        self.assertEqual(os.listdir(self.lock_dir), [], "the third waiter's lock was clobbered")
        graves = [n for n in os.listdir(self.tmp) if n.startswith("lock.reclaim.")]
        self.assertEqual(len(graves), 1, graves)
        with open(os.path.join(self.tmp, graves[0], "pid"), "r", encoding="utf-8") as f:
            self.assertEqual(f.read().strip(), "424242")

    def test_reclaim_hook_ignored_outside_test_mode(self):
        self._write_lock(time.time() - 600, pid=111111)
        self._set_netstat([])
        proc = self._run(
            "start",
            env_overrides={
                "DB_TUNNEL_TEST_MODE": "0",
                "DB_TUNNEL_LOCK_WAIT_SECS": "1",
                "DB_TUNNEL_TEST_RECLAIM_HOOK": self.reclaim_hook,
                "DB_TUNNEL_TEST_HOOK_PLAN": "fresh",
            },
        )
        self.assertEqual(self._hook_phases(), [], "the hook ran without DB_TUNNEL_TEST_MODE=1")
        self.assertIn("removing stale start-lock", proc.stderr)

    def test_release_leaves_a_lock_held_by_another_process(self):
        # While start runs, its lock is replaced by another process's (pid 424242). When start
        # returns, release_lock must not delete a lock it does not hold.
        self._set_netstat([])
        proc = self._run("start", env_overrides={"DB_TUNNEL_TEST_FAKE_SSH_MODE": "steal_lock"})
        self.assertEqual(proc.returncode, 1, proc.stderr)
        self.assertEqual(len(self._ssh_calls()), 1)
        self.assertTrue(os.path.isdir(self.lock_dir), "release_lock deleted another holder's lock")
        with open(os.path.join(self.lock_dir, "pid"), "r", encoding="utf-8") as f:
            self.assertEqual(f.read().strip(), "424242")
    ...
    def test_listener_named_like_ssh_but_not_ssh_is_not_killed(self):
        self._write_state(owner="manual", winpid=9001)
        self._set_netstat([NETSTAT_LINE % "9001"])
        for row in (PS_LINE_SSH_AGENT, PS_LINE_SPACED_SSH_EXE):
            with self.subTest(row=row):
                self._set_ps([row % (66, "9001")])
                proc = self._run("stop", "--force")
                self.assertEqual(proc.returncode, 2, proc.stderr)
                self.assertIn("NOT an ssh process", proc.stderr)
                self.assertEqual(self._kill_calls(), [])
