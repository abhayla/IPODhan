...
reconcile() {
  ...
  while read -r num br; do
    ...
    # Never --delete-branch here: when the merge call fails (auto-merge is disabled on this repo,
    # or mergeability is still UNKNOWN) gh still deletes the head branch and GitHub CLOSES the PR
    # unmerged (2026-09-07: #381, #383, #395 closed seconds after creation). Arm only; the helper
    # that lands a PR deletes the branch after state == MERGED.
    gh pr merge "$num" --auto --squash >/dev/null 2>&1 && { echo "armed #$num ($br — lands when required CI passes)"; any=1; }
