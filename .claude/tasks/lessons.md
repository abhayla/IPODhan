...
- 2026-09-16 (item 7 / deploy suite): a suite printed 179 raw PASS lines against an earlier 188 on unchanged code, reading as nine lost assertions. It was a COUNTING ARTIFACT: those lines embed mktemp paths, commit shas and pids, so `sort -u` collapses them differently per run. Normalising the volatile fields gives 188 unique assertions in both runs. Never diff raw suite output between runs, and never treat a changed `grep -c` as evidence about coverage - compare the suite's own terminal line and exit code, or count case identifiers (`case 12:`), not whole lines. Related trap the same night: `grep -c "^FAIL"` as the LAST command in a pipeline exits 1 when it finds nothing, so a clean suite reports failure.
- 2026-09-16 (item 7 / by-construction claims): THREE claims of the form "this case is safe because X" arose in one slice and TWO were wrong when tested. The anchor budget survived, and only because the builder ran the REAL exported maxAnchorSpawnsWithinLockTtl rather than re-implementing its arithmetic (a re-implementation can agree with your assumption and disagree with the code). A "surviving" mutation was actually a duplicated export, so the survivor was a claim about the MUTATION, not the test. And an exemption - "resume_scraper needs no preflight, it resumes against the previous already-built release" - failed the moment a test was written to pin it: SCRAPER_RESUME_TARGET is set to "new" at the atomic flip (deploy-linux.sh:1724) BEFORE restart_pm2 runs, so a deploy aborting in between resumes against the NEW release unchecked. RULE: in a slice that removes a safety mechanism, a "safe by construction" claim gets a pinning test or is treated as UNPROVEN. A comment explaining an exemption is not a check - it cannot fail when the premise changes underneath it.
