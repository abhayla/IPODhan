...
# Repo-level scripts and gates (run from root)
node --test scripts/tests/<name>.test.mjs         # scripts/ self-tests use node:test, not vitest
node scripts/build-detection-registry.mjs --check # detection registry aggregates not stale (CI gate)
cd scraper && npm run type-check:scripts          # the only type-check scraper/scripts gets
npm run docs:verify                               # regenerate rule index, registry, board; design-consistency check
```
...
│   └── src/db/schema.ts              # DB schema (~43 tables) - SINGLE SOURCE OF TRUTH
...
- **Deploy:** GitHub Actions `deploy-linux.yml` (`workflow_dispatch` only, `slot=staging|prod`), on the
  `linux-vps-ipodhan` self-hosted runner. A merge to `main` never deploys anything; staging deploys
  come from VPS-cron windows (`mode=window`) or a manual dispatch.
...
- **CI: what runs automatically.** On a PR: `pr-gate.yml` (code PRs; lint, type-check, unit tests plus
  dozens of named repo gates — migration journal/snapshot chain, write ratchet, module boundaries, design
  traceability, `detection-change-gate`, `python-tests` for hooks, `web-build`), `secret-scan.yml`, and
  `docs-gate.yml` (docs-only PRs). On push to `main`: `main-gate.yml` (lint:ci + unit + registry check).
  `ci.yml`, `test.yml` and `prod-verify.yml` are `workflow_dispatch`-only — full integration/E2E do NOT
  run automatically; dispatch them or run the suites locally for non-trivial changes.
- **A new file under `scripts/tests/` is not run by CI** unless `pr-gate.yml` names it — the gate lists
  test files explicitly. Grep `pr-gate.yml` for your file before trusting a green check.
- **CI skips a CONFLICTING PR entirely** (zero runs). No checks on a PR means rebase it, not "CI broken".
...
- `docs/design/data-sourcing-pull-model.md` - **the requirements spec (SSOT)** for data sourcing; decision
  log in §0.0.1 (OD rows). Grep it by the subject's key terms before recommending anything.
- `docs/design/pull-model-completion-state.md` - built / partial / not-built inventory; no prod deploy
  while it lists anything partial (`.claude/rules/staging-is-the-release-gate.md`)
- **Admin data editing** (owner's IPO-correction feature): spec **§9** of `data-sourcing-pull-model.md`,
  decisions OD-102..OD-121, findings F-168..F-174. Not built yet; read §9 before any admin work.
- `docs/design/findings.json` - proven research findings (F-ids); `docs/reviews/failure-classes/` +
  `docs/reviews/detection-checks/` - one JSON per defect class / check, aggregates regenerated by
  `scripts/build-detection-registry.mjs` (see `docs/reviews/README.md`)
- `docs/ops/prod-ops-recipes.md` - staging/prod read and repair recipes
