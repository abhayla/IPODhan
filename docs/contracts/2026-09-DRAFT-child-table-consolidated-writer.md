...
> 4. Format `[YYYY-MM-DD HH:MM] <STAGE|PROGRESS|DEFECT|EVENT|DECISION|REVIEW|BLOCKER|DONE> — <summary>`. Timestamps come from `date` in the same command, never estimated — estimates drifted eighty minutes on 2026-09-05.
...
**Proof:** one staging scraper cycle after the flag is on, showing per-field `field_sources` rows for a child table that previously had exactly one synthetic `rows` entry. The counter that must move: `select table_name, count(*) from field_sources where ipo_id = '<a staging IPO with financial_statements rows>' group by table_name` goes from one row named `rows` to one row per field per fiscal year. Read it in the cycle AFTER the one that writes it, so a value that a later cycle overwrites is caught.
...
- One prod deploy per project per day, in the evening window, and only from a frozen `release/prod-<date>` branch. This run does not deploy.
