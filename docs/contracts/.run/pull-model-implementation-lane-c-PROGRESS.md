...
would send a resuming reader to the wrong place. A NEW file dropped in this directory would
still be ignored.) The other durable records are
...
### 2026-09-16 05:25 IST — #688 merged: the register reader now reads to the end; two corrections

The report-82 reader fix (#686) is on main after a Tier B review that caught an untested dedupe path; both gaps got tests that fail on their mutants. Corrected numbers: the old cap was **100 rows** per year and category, and FY2025-26 SME holds **267**. The amendment worktree is gone: `main checkout: tracked 4823 -> 4823, dirty 18 -> 18, deleted-on-disk 0`.

Correction on #682: the status updater does run every cycle but compares a UTC day against Indian dates, so flips land 5.5 hours late; the four rows self-correct after 05:30 today and the fix is #689 (lane B). The user-visible effect on prod stands.

PR B (the STANBIK class repair) is in build on top of #688. Eight PRs merged tonight; no production data written.



### 2026-09-16 05:47 IST — the STANBIK class repair is built and applied on staging (PR #692); two more defects filed; one supervision miss of mine

The repair tool finds every IPO row storing its face value as the price band, resolves the real issue price from three years of Chittorgarh's register, and repairs only rows with a source. On both slots the class is five rows; STANBIK resolves to ₹30 with a face value of 10, the other four have no source and are untouched. Staging apply wrote STANBIK only; its issue-size floor violation is gone, and the lot-band check still names it because its lot size (100) looks wrong too, a separate field for a separate slice. Tier A review running; prod apply is the owner's word.
...
### 2026-09-16 06:47 IST — the STANBIK repair is HELD on two scheduled wakes; the 06:16 line counted a restart and is superseded

| marker (IST) | trigger | count |
|---|---|---|
| 06:05:56 | deploy restart (#680) — not counted | 0 |
| **06:14:09** | **scheduled :15 wake** | 0 |
| 06:23:19 | deploy restart (2d77e0cd) — not counted | 0 |
| 06:37:10 | deploy restart (6ab8a5ed) — not counted | 0 |
| **06:45:11** | **scheduled :45 wake** | 0 |

Two scheduled wakes at zero: **HELD**. The tool itself counts restarts as cycles and printed HELD on two of them, which is #698. The production apply for STANBIK is now a recommendation (brief decision 11). Lane C's night is closed: nine PRs merged, nine issues filed, no production data written, nothing further unblocked without the owner or the release cut.



### 2026-09-16 08:50 IST — LANE C CLOSED (owner decision 08:58 IST)

Items 12 and 2 proven on staging, item 14 blocked on decision 4, production untouched, decisions 4/8/11 with the owner. Nine PRs merged, nine issues filed. The ledger tree stays; the stale slice-7 tree from 2026-09-11 holds unmerged work and remains the owner's call. Quiet from here.
