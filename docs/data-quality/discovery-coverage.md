...
| 7 | Two different companies carrying byte-identical figures to the rupee (T-291 P3-9) | D4 whole-table invariant (cross-row equality) | Per-IPO review reads each row as internally plausible | **NO** — issue #178 open, root cause unresolved. **Gap G-C** (#188) |
...
| 16 | Sitemap omits 4 live landing pages; duplicate slugs both indexed; reviews pages serve the generic homepage `<title>` (T-272 P2-4/P2-3, T-264 P3-5) | **D16 SEO-surface completeness** | Nothing links the route inventory to the sitemap; each looks fine alone | **NO**. **Gap G-E** (#190) |
...
| 20 | `data_conflicts` grew unbounded (11,493 rows), the same conflict re-inserted per cycle, `resolved_at` never set (T-285 P2-3) | D4 invariant (table growth) | Invisible from the product; only a row-count trend shows it | **PARTIAL** — inflow cut ~86% (T-286); no standing unbounded-growth check. **Gap G-F** (#191) |
| 21 | Conflict detection **inert**: `conflictsDetected: 0` every cycle while real disagreements existed (T-272 P3-5) | D6 log-pattern contradicted by D4 | A zero looks like health. Only cross-checking against a known-true disagreement exposes it | **PARTIAL** — cause fixed; no standing "detector reported 0 for N cycles while invariant X is violated" cross-check. **Gap G-F** (#191) |
...
| 23 | A repaired value **rebounds on the next cycle**, through a different mechanism each time (T-281, T-287C2/F2/F3, T-293C) | **D7 hold-through-cycle** | "Merged, deployed, row now correct" is not evidence. Only surviving a real cycle is | **NO** standing post-repair hold assertion. **Gap G-G** (#192) |
...
| 38 | **No workflow runs the full scraper suite** — CI green does not mean the changed scraper code was exercised (T-259C F1) | D9 liveness applied to CI itself | The check is named "Unit Tests" and it is green | **PARTIAL** — `pr-gate.yml` runs *targeted* scraper regressions, not the full suite. **Gap G-M** (#198) |
...
Filed 2026-08-23 as issues **#186–#199**. Every one carries an exact specification —
the query, the assertion, the schedule, and the deliberate exceptions — in its issue body.
No gap is left unlisted.

| ID | Gap | Layer | Method |
|---|---|---|---|
| **G-A** ([#186](https://github.com/abhayla/IPODhan/issues/186)) | Serving-vs-storage row-count delta for every list API (`/api/registrars`, `/api/ipos`, holidays, NCD, OFS, rights) | serving | D5 |
| **G-B** ([#187](https://github.com/abhayla/IPODhan/issues/187)) | **Reverse sweep**: every IPO on an external calendar must exist, be typed `IPO`, and return 200 on our detail route | data + render | D2 |
| **G-C** ([#188](https://github.com/abhayla/IPODhan/issues/188)) | Provenance gate: a row asserting a hard date or band with **zero** `field_sources`, or two rows with byte-identical figures, fails the gate | data | D11 |
| **G-D** ([#189](https://github.com/abhayla/IPODhan/issues/189)) | Cache-poisoning probe: bisect the default list parameter and assert the default page's dataset matches its neighbours | serving | D5 |
| **G-E** ([#190](https://github.com/abhayla/IPODhan/issues/190)) | SEO-surface completeness: sitemap ⊇ every live 200 route; no duplicate-slug pair indexed; every page type has its own `<title>` | SEO | D16 |
| **G-F** ([#191](https://github.com/abhayla/IPODhan/issues/191)) | Pipeline-signal health: repeated-WARN-per-cycle detector, unbounded-table-growth alarm, and an inert-detector cross-check | scraper | D6 |
| **G-G** ([#192](https://github.com/abhayla/IPODhan/issues/192)) | Post-repair hold assertion: after any data repair, re-assert the invariant following at least one real scraper cycle before the task closes | data + scraper | D7 |
| **G-H** ([#193](https://github.com/abhayla/IPODhan/issues/193)) | Liveness of rollout flags: assert every `*_PERCENTAGE` gating live logic is non-zero in production (the T-282 inert-pipeline class) | config | D9 |
| **G-I** ([#194](https://github.com/abhayla/IPODhan/issues/194)) | Deployed-SHA lineage: assert the deployed SHA is an ancestor of `origin/main` (the exact hole T-264 P2-4 fell through; still open) | deploy | D10 |
| **G-J** ([#195](https://github.com/abhayla/IPODhan/issues/195)) | Alert-channel quality: standing signal-to-noise measurement plus a periodic synthetic-alert drill proving the notify path is live | monitoring | D12 |
| **G-K** ([#196](https://github.com/abhayla/IPODhan/issues/196)) | Standing mutation job over the declared guard set — a guard that survives deletion with a green suite fails the job | meta | D8 |
| **G-L** ([#197](https://github.com/abhayla/IPODhan/issues/197)) | Test-coverage negative grep: every shipped data-table component and every named validation rule must appear in at least one test | meta | D14 |
| **G-M** ([#198](https://github.com/abhayla/IPODhan/issues/198)) | Run the **full** scraper suite in CI, not a targeted subset (T-259C F1, still true) | meta | D9 |
| **G-N** ([#199](https://github.com/abhayla/IPODhan/issues/199)) | Forward external-oracle compare as a standing check, using an oracle the scraper does **not** ingest | data | D1 |
