...
- 2026-09-10 19:07 IST — Still 12 of 19 pieces (63%), still none proven, and the server is still full: 787MB free, same 15 old folders, nothing cleared. A reader of ipodhan.com would notice nothing; the live site is fine. One detail worth your eye: the free space is still creeping DOWN even though no deployment has run for hours, so something else on that machine is writing and there is no room left to absorb it. Nothing went wrong this half hour — three finished pieces are sitting built and waiting, and I worked out the order they have to go in so one of them does not block itself. What I need from you is unchanged and is the only thing that moves anything: run the clean-up in issue #505, then add the database index.

- 2026-09-10 18:59 IST — Still 12 of 19 pieces (63%), unchanged, and still none of it proven. The server is still full: 788MB free, the same 15 old test-deployment folders, nothing cleared yet. A reader of ipodhan.com would notice nothing — the live site is fine. One thing did go right: stopping the merges worked. No new deployment has been attempted since 12:26, so no new 3GB folder has been created, and the hole has stopped getting deeper. It is not getting shallower either, and it cannot until you run the clean-up. What I did meanwhile: built one more safety check (held, not merged) that catches a specific way the next piece of work could quietly corrupt how we recognise duplicate company names, and found a real trap in it — the shortcut version of that name function, written in database language, disagrees with the real one on junk names like '---'. Using the shortcut for the repair would have created exactly the collision the repair exists to prevent. Written down before anyone hit it. What I need from you is unchanged and is now the only thing moving: run the clean-up in issue #505, then add the database index.

- 2026-09-10 18:44 IST — **STOP THE LINE. The server is out of disk.** Item 1 is unchanged at 12 of 19 pieces (63%) and none of it is proven, because the test site has not deployed since 07:40 this morning. A reader of ipodhan.com would notice nothing — the live site is up and unaffected — but the machine it lives on has 788MB free out of 96GB, and that is close enough to breaking things that it needs you tonight. What went wrong: each time a test deployment fails, it leaves behind a 3GB folder, because the clean-up only runs when a deployment succeeds. Twelve failed today, so 37GB of dead folders. Every merge to the main branch makes another one, so I have told all three work streams to stop merging until it is cleared. I also have to correct myself: the database index I asked you for is no longer the thing blocking the test site — the deployment now stops at the disk check before it ever reaches that. What I need from you: clear the old test-deployment folders (issue #505 has the exact command, which reads the live pointer first so it cannot delete the one in use), then the index. I have not touched the server beyond reading it.

- 2026-09-10 18:12 IST — Item 1 went from 6 of 19 pieces done to **12 of 19 (63%, was 32%)**. A reader of ipodhan.com would notice nothing yet: every piece so far is plumbing under the site, and none of it is on the live site — it is all waiting on staging, which has been down since 08:11 this morning. What went wrong: the test step that checks the website's data pages had been crashing before it ran anything, and reporting the crash as a pass, because a change of mine dropped three settings it needed; that is fixed and the check now really runs, with the 25 genuinely failing files written down so the number cannot grow. A second problem was caught before it shipped: two pieces both numbered their database change '36', which would have made the database skip or repeat a step. What I need from the owner: one line run against staging — `CREATE INDEX IF NOT EXISTS idx_ipos_slug ON ipos (slug);` — nothing else unblocks it.
...
- 2026-09-10 17:56 IST — Three more merged. Facts now record which row they came from, not just which IPO (#459). Feature flags can default differently on staging than on production, so a new flag can be tried on staging without touching live (#476). And the web integration test gate is finally alive: it shipped with an empty list, which meant it could not catch anything; it now carries the 25 test files that really fail, measured from the first CI run where that step actually executed (#500). The old "26 of 44 failing" figure is withdrawn — it came from a run that crashed before testing anything. Filed #501: two old database-migration records are dated out of order, harmless today but it makes "when did this land" unanswerable.

- 2026-09-10 19:01 IST — Disk relief merged (#547). Every release we deploy carried a 1.5GB build scratch folder that nothing ever serves; it now lives in one shared place per environment, so each release is roughly half the size and builds reuse the cache instead of starting cold. That is the same disk that filled to 100% yesterday and stopped all three work streams for two hours. Two other changes are in the queue and neither is being waved through: the deploy-cadence change (#549) was put back on top of today's latest code before merging, because it edits the same CI file another change had just touched; and the build-safety check (#548) is red on CI for a real reason — the test I asked for uses a feature that only exists on a newer version of Node than CI runs, so it passes on my laptop and can never pass in CI. Being fixed, not silenced. On the provenance check I reviewed tonight I found a defect the author had not: it works out a row's identity with the wrong function, so a company whose name is pure punctuation would be reported as broken when it is fine. Latent today, would start crying wolf the day the next slice lands. Fixed before the review even opens.
