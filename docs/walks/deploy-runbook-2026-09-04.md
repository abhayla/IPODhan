...
- Copy the 6 DEEPA PDFs from the laptop: `scp -i ~/.ssh/firekaro_v6_vps "C:/Users/itsab/AppData/Local/Temp/claude/D--Abhay-Ventures-IPODhan/874ccc0d-4cd3-4fbf-b74d-34194bc2ff71/scratchpad/store/0b7e81cd-3426-4376-9bc8-1b3b07fa9a93/"*.pdf root@72.61.240.224:/var/www/ipodhan/shared/prospectus/prod/<DEEPA prod ipo id>/`
  (DEEPA's PROD ipo id differs from the test DB id: read it first: `select id from ipos where slug='deepa-jewellers'` via the app's admin API or a read-only psql from the VPS).
...
- `ENABLE_DATA_CONSOLIDATION=true ENABLE_SOURCE_TRACKING=true ENABLE_CONFLICT_DETECTION=true CONSOLIDATION_PERCENTAGE=100 npx tsx scripts/persist-filing.ts --ipo deepa-jewellers --json-ad /tmp/deepa-ad.json --json-rhp /tmp/deepa-rhp.json` (dry run) then `--apply`.
