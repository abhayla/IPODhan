...
| status-restricted run drops newly announced IPO | scraper/src/base/BaseScraperOrchestrator.ts (S-02 §5 `allowedStatuses` guard) | a newly announced IPO (UPCOMING/OPEN) first seen during a status-restricted run is silently skipped and NEVER created by a later cycle either, because there is never an existing row for the guard to match against; under the current due-step cadence this is not an edge case — Chittorgarh/Moneycontrol have NO unrestricted run once `ENABLE_DUE_STEP_SCHEDULER` is on (`docs/scraper/scheduler-liveness.md`), so every discovery of a new IPO went through this path and was dropped. 8 rows confirmed missing on prod as of 2026-09-07 (advenzymes, infrax-renewable-ltd, karamtara-engineering-ltd, manipal-payment-and-identity-solutions-ltd, rentomojo-ltd, sis, steamhouse-india-ltd, vinod-texworld-ltd) — of these, `sis` and `advenzymes` are rejected upstream BY DESIGN (`data-validation.ts` `NON_IPO_SCRIP_CODE_NAME`/`NON_IPO_WINDOW_TOO_LONG`, ~line 330/393: bare scrip-code names and no-substance long windows), so this fix creates at most 6 of the 8 | 2026-09-03 (staging-vs-prod discovery lag first observed on Karamtara Engineering, issue #351) | this PR (#351 fix — the guard now allows a brand-new row through when the row's OWN validated status, the same status the write path would persist, falls inside `allowedStatuses`; existing out-of-scope rows and new out-of-scope rows both still skip) | none yet — register as **G-B (#187)**, the standing prod-vs-staging / Chittorgarh-upcoming reverse-sweep check, as the detection to build (a prod UPCOMING/OPEN set vs the Chittorgarh upcoming list, or vs staging) | unguarded |
