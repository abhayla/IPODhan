...
| 7 | Job scheduler + budgets | **PARTIAL** | Scheduler built (`scheduler/`, `due-step-cycle.ts`). OD-55 force-kill removal merged but **not on prod** (#805). Tiering (O-4) unverified. Staging proof (2026-09-23, release 084928e0): S1 — the 22:20 IST live wake logged "Live-figures job: took scraper:live (independent of scraper:cycle, OD-27)" with ttlMs 240000, never took scraper:cycle, logged "GMP record created successfully" 23 times (17 rows carry a 22:xx source time; F-143), and released scraper:live. S2 — the 22:15 IST non-slot data wake skipped discovery and the document cycle as designed, but the website list scrapers still ran (F-142; slice S2b). S3 — closed-IPO cron lines installed at 25,55 22,23; ENABLE_CLOSED_IPO_JOB not set (off). Staging check (2026-09-24 08:41 IST, FAILED, release 084928e0): the non-slot skip holds (22:15-23:45 wakes skipped the document cycle; the 14:00 slot closed 21:45), but the 00:00 slot never closed - "incomplete: pass=discovery,listed_deferred" on all 16 wakes 00:15-07:45 because the document retry timer makes the same documents due every wake and the 60 s discovery budget runs out (F-151, #943). **S4 opening-day check (OD-31), PR #949, fixed after an independent review FAILED it (5 findings) — `--job=opening`, prod cron 45 9 * * *, staging 40 9 * * * (finding 4: 47 was never actually free either; :40 is the real gap in staging's :15/:45 data, :20/:50 live, :25/:55 closed minutes). Fix per finding: (1) CRITICAL — the gate no longer depends on a pre-existing DB row; the lock is taken, BOTH exchange lists are fetched unconditionally, and `anyIpoOpensToday` is read AFTER the fetch+write (class-covered: no row, NULL open_date, stale/postponed open_date — all now caught); a pure `opensTodayFromFetch` predicate is unit-tested for the same three cases. (2) `discoveryOnlyMode()` added to `BaseScraperOrchestrator` (mirrors `liveFiguresOnlyMode()`) — narrows the write to `companyName/symbol/isin/segment/offeringType/sourceKeys/status/openDate/closeDate` before the protection filter AND the verifier-hint write, proven by a new orchestrator-level test that a fetched row carrying price band/lot size/issue size/registrar/etc. writes NONE of them (T-403 r5's own "hint written on every exit" invariant kept intact for the non-discoveryOnly path). (3) mutation-proof done: `opensTodayFromFetch`'s IST computation mutated to UTC, 5 tests went red, restored, tests green again. (4) staging cron minute moved 47->40. (5) the lock-held skip log no longer computes a start time from TTL (the lock stores no start time and the data job renews the TTL every 5 min, so the old computation was plausible but wrong) — logs "held (start time unknown)". All unit + wiring + shell tests green (63/63 relevant suite + scraper-wake.test.sh all cases); Core proof re-validated against the fixed gate — still not proven on staging.** |
