...
| 12 | Name normaliser + duplicate detection | **PARTIAL** | Built: `company-identity-fold.ts`, `company-name-normalizer.ts`, `company-name-similarity.ts`, `ipo-identity.ts` (OD-34 binding order). **Reopened 2026-09-23 by OD-68 to OD-71 (R-191 to R-197, §2.3.3.2):** a live duplicate exists on production AND staging (`rays-of-belief-ltd` / `rays-of-belief-ltd-o`, Rs 74.99 cr vs Rs 125 cr, created 2026-09-01), so matching still lets one IPO become two rows. Missing: status-suffix and page-title stripping before matching, hold-for-review instead of a new row, the merge tool's refusal on differing identifiers or open dates, the look-alike test pair, and §2.3.3.1's standing nightly sweep (being built, #903). Residual: #679 |
...
| 17 | Closed-IPO job (OD-22) | **PARTIAL** | **Runs, but cannot do its job (#717, measured 2026-09-23).** First real cycle on staging (owner decision A): 10 closed IPOs selected, all 10 recorded DONE with `fieldsWritten 0` / `NO_DUE_FIELDS`, because `resourceClosedIpo` only runs `walkFieldPlanForIPO` and the selected IPOs have 0 `ipo_field_plan` rows; the stuck PROSPECTUS documents that selected them stayed PENDING (74 before, 74 after). DONE rows are never re-picked (`closed-ipo-job.ts:108-113`). Flag turned off again the same cycle. Built and present on `refs/remotes/origin/main`: the job, `isClosedIpoJobDue` (22:00 IST boundary), `closed_ipo_resourcing` + migration 0050, `runStep(cycleId, 'closedIpoJob', ...)` at `index.ts:1092`, flag `ENABLE_CLOSED_IPO_JOB` (default false). Missing: the job must extract the pending documents it selects an IPO for, and a zero-work pass must not be DONE; repair of the 10 wrong DONE rows on staging. Registry: `done-recorded-when-worker-found-nothing`. |
...
| 19 | Merge tool on shared write path | **PARTIAL** | Singular tool routed (#432). Plural clustering tool (`merge-duplicate-ipos.ts`) routed too — raw-SQL apply block replaced with `IPORepository.mergeDuplicateInto`, ratchet entry removed (piece 1 of #807). **Piece 2 LANDED 2026-09-23 (#888): the merge log exists.** `ipo_merge_log` (migration 0051) snapshots the consumed `ipos` row, both ids and slugs, the survivor patch, and child-row counts split deleted-vs-repointed — written INSIDE the merge transaction and BEFORE the delete, from a row re-read under `FOR UPDATE` so the snapshot is taken at delete time rather than from the read made before the transaction opened (a Tier A review found that gap; without the lock a concurrent write would be deleted AND missing from the log). Proven on real data by `scripts/assert-merge-log-restores.mjs` — a real merge against `ipodhan_test`, 14 checks, all 56 declared columns round-tripping byte-for-byte including two the survivor did not carry. **#807 remaining: `unmerge` itself, which now has a source of truth to restore from.** Two limits named rather than hidden: child ROWS are not captured (counts only — they are re-derivable scraper data), and the snapshot covers every column `schema.ts` DECLARES, which on staging is six short of the live table (all six 0/379 non-null today, and the proof script FAILS if one starts holding a value) |
