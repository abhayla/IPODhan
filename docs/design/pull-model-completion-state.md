...
| 6 | The pull walk over the plan | **PARTIAL** | `field-plan-walk.ts` + **3** fetchers registered at `field-plan-walk-deps.ts:192-194` (DOC, BSE, CHITTORGARH). #705 re-measured 2026-09-20 and is **stale in its premise** - the walk does have fetchers. What it correctly names: **no NSE fetcher**, so NSE-ranked timetable/status/subscription fields hit `NO_FETCHER_REGISTERED` and back off (same gap as #759). #762: 12,480 plan rows parked with no re-queue path (fix merged as #763, **not on prod**) |
...
| 10 | Verification checks of section 4 | **PARTIAL** | 8 `audit-*` scripts exist. **12 of the 21 paper checks wired and merged 2026-09-20 (#859)**, 13th in #863 — they found four live defects on their first runs (#858 retired listing prices, #860 null segments, #861 a check that never reports, #862 79 E-1 writes from DRHP). **6 of the remaining 8 are BLOCKED on mechanisms that do not exist** (measured, see #778): `pull_frozen` needs supersession (zero callers, no `superseded_by` column); the three `reread_*` need item 9; `pull_noblank` needs a current-value read across 190 columns; `pull_noop_suppression` needs its counters persisted. Real deliverable is 13-15 checks, not 21. |
| 11 | Crore conversion (OD-20) | **NOT BUILT** | No `repair-*crore*` tool in the tree. Spec calls it its own release, not cleanly reversible. **Scope measured 2026-09-20 (#854): exactly 5 of the 37 CRORE-class columns move** (`ipos.issue_size`, `ipo_details.fresh_issue`/`ofs_issue`, `ipo_valuation.mcap_at_floor`/`mcap_at_cap`); 25 are already crore and 7 are per-row-unit, and touching them divides by 1e7 twice. **Collides with item 14 on `ipos.issue_size` — do not run both at once.** Card carries two unclosed MAJORs (F-95, F-77) |
...
| 14 | BSE share count to rupees | **PARTIAL** | `computeBSEIssueSize` at `bse-api-scraper.ts:148`. **#728: issue size is 41-76% LOW on 6 of 6 live IPOs.** **Collides with item 11 on `ipos.issue_size` (#854): a unit conversion and a correctness repair must not be in flight together, and a repair written against rupees breaks once the column holds crore.** The spec sizes this Tier C, verification not construction - that sizing is wrong by a tier |
...
**Totals: 16 built, 10 partial, 3 not built, of 29 items.** (Counted from the table itself. An
