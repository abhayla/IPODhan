...
Every card under `docs/design/build-cards/` carries these THIRTEEN headings, spelled exactly as
...
The design rule ids (`R-nnn`, from `docs/design/rules.json`) this item implements, grouped by design
section. Generated by `node docs/design/apply-rule-ownership.mjs --apply` from the section-to-item
map in `rule-ownership.json`, and editable by hand afterwards — check **D19** reads the CARD, not the
map, so a deliberate correction here wins. An item that implements no numbered rule says so in a
sentence; an empty list and a forgotten list look identical.

## Known gaps

What this item does NOT close, with the finding id and the reason. This heading exists so that
"zero open findings" can never be reached by quietly dropping one: a finding moved here is still
visible, still owned, and still has a name.
