...
Status: DONE 2026-09-24 PRs #910, #925, #966, #976, #984, #1001 proof docs/design/pull-model-completion-state.md row 12: company-identity-fold.ts, company-name-normalizer.ts, company-name-similarity.ts, ipo-identity.ts, identity-decoration.ts on refs/remotes/origin/main; merge gate refuses differing offering_type always / differing close_date+listing_date unless OD-86 (#1001, 97b7f4b4, OD-94); the live rays-of-belief-ltd duplicate merged on staging with a slug redirect; decorated-slug renames applied on staging (5/5, 0 collisions); SEBI draft-number step dropped by OD-89 (no per-offering number exists, F-125). Residual, not blocking this item's own scope: #1002 (26 pre-merge-log staging merges, one wrong) and #1003 (raw-SQL merge path sweep)
...
32 rule(s) from `docs/design/rules.json`, generated by
`node docs/design/apply-rule-ownership.mjs --apply` from `rule-ownership.json`.

(2026-09-24: R-223 claimed by hand — new rule id from the OD-85 build-clarification edit;
`apply-rule-ownership.mjs --apply` was not run per this task's constraint.)
