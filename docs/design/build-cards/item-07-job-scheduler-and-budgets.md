...
| `scraper/src/services/document-state-machine.ts` | exists | **As built (S3, F-151, #943; §2.1 "The timed backoff retry is removed", OD-21, OD-33):** `RETRY_MINUTES` (30/60/30/360 min) and `computeNextRetryAt` are deleted. `nextAttemptSlotStart` writes `next_retry_at` = the start of the NEXT OD-19 data slot (`nextDataJobSlotBoundary`), never `now + N minutes`; `attemptedThisSlot` skips an open row whose `next_retry_at` is a future slot start. A `next_retry_at` that is not a slot start (written by the removed ladder, pre-deploy rows) is not honoured: the row is due on its first wake after deploy. An open document is therefore attempted at most once per data slot; a stage change makes the new stage's types due at once (missing rows). `NOT_FOUND_MAX_ATTEMPTS` (5) now counts slots. No stage-at-attempt column exists, so an already-open row is not re-attempted mid-slot on a stage change; it waits for the next slot. |
| `scraper/src/services/document-cycle.ts` (S3) | exists | **As built (S3, F-151):** `listed_deferred` is no longer an `incompletePasses` entry — the data slot can log `complete` while LISTED rows are deferred (§6.1 gives old IPOs to the 22:00 closed-IPO job). `listedDeferred` stays on the summary line. |
...
| `packages/shared/src/repositories/ipo-field-plan-repository.ts` + `scraper/src/services/field-plan-walk.ts` | exists | **As built (S3, F-152):** `FIELD_PLAN_BACKOFF_*` and `fieldPlanBackoffMinutes` (15 min doubling to 6 h) are deleted. `recordOutcome` writes `next_due_at` = next slot start for a non-terminal row, NULL for a terminal row and for a structural gap row (`gapKey`, no attempt charged, re-asked only when its gap key changes). The walk logs an all-structural failure (`FIELD_PLAN_GAP_CODES`: NO_MAPPING, NO_DOCUMENT_PROVENANCE, NO_FETCHER, NO_DOCUMENT_TYPE, COLUMN_READ_NOT_IMPLEMENTED) as a STRUCTURAL gap, not TRANSIENT; a mixed genuine+structural failure stays on the transient path (next slot). Out of scope, still a timer: `filing-auto-persist.ts` extraction-failure backoff (`backoffNextDueAt`), which re-extracts a stored document, not a fetch. |
