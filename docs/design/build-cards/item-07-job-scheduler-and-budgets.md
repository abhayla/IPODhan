...
| `scraper/src/services/document-state-machine.ts` | exists | **As built (S3, F-151, #943; §2.1 "The timed backoff retry is removed", OD-21, OD-33):** `RETRY_MINUTES` (30/60/30/360 min) and `computeNextRetryAt` are deleted. `nextAttemptSlotStart` writes `next_retry_at` = the start of the NEXT OD-19 data slot (`nextDataJobSlotBoundary`), never `now + N minutes`; `attemptedThisSlot` skips an open row whose `next_retry_at` is a future slot start. A `next_retry_at` that is not a slot start (written by the removed ladder, pre-deploy rows) is not honoured: the row is due on its first wake after deploy. An open document is therefore attempted at most once per data slot; a stage change makes the new stage's types due at once (missing rows). `NOT_FOUND_MAX_ATTEMPTS` (5) now counts slots. Round 2 (Tier A MAJOR): a LISTED IPO's open row is attempted ONCE after the IPO entered LISTED and again only when a new document row appears or a document for the IPO is first seen after the row's last attempt (`listedRowDue`; OD-56, §2.5.1, OD-81 event 2). "Attempted since entering LISTED" is the new column `document_fetch_state.attempted_at_stage` (migration 0054, additive, nullable), written by the runner only for a chain that concluded (never `chain_incomplete`, never the rotation stamp); `listing_date` is deliberately NOT used to infer the stage change (#932). |
| `scraper/src/services/document-cycle.ts` (S3) | exists | **As built (S3 round 2, F-151):** `listed_deferred` IS an `incompletePasses` entry, but `listedDeferred` now counts only LISTED IPOs with a DUE row (not yet attempted at LISTED, or a new document since), so the due set shrinks every wake and the slot logs `complete` once it is empty. Round 1 cited "§6.1: the LISTED backlog belongs to the 22:00 closed-IPO job" — **that citation was wrong**: §6.1 says the closed-IPO job runs "exactly the pull walk of §2.4" over field plans, and `closed-ipo-job.ts` never reads a document, so the data-slot document cycle is the ONLY job that fetches LISTED documents. The LISTED enrichment passes the newest `documents.uploaded_at` to `planIpoCycle` for the OD-81 new-document event. Convergence: at the default cap 2 IPOs/wake and 16 wakes/slot, 32 due LISTED IPOs finish per slot; staging's one-time backlog (52 LISTED IPOs with BLOCKED_ALL rows, of which only those inside the 10-day live window are candidates) needs at most ceil(52/2) = 26 wakes = 2 slots, after which each row is attempted and no longer due. |
| `packages/shared/src/scheduler/data-job-slots.ts` | exists | **As built (S3):** adds `nextDataJobSlotBoundary` and `isDataJobSlotBoundary`, the only "when next" the document and field-plan paths write. |
| `packages/shared/src/repositories/ipo-field-plan-repository.ts` + `scraper/src/services/field-plan-walk.ts` | exists | **As built (S3, F-152):** `FIELD_PLAN_BACKOFF_*` and `fieldPlanBackoffMinutes` (15 min doubling to 6 h) are deleted. `recordOutcome` writes `next_due_at` = next slot start for a non-terminal row, NULL for a terminal row and for a structural gap row (`gapKey`, no attempt charged, re-asked only when its gap key changes). The walk logs an all-structural failure (`FIELD_PLAN_GAP_CODES`: NO_MAPPING, NO_DOCUMENT_PROVENANCE, NO_FETCHER, NO_DOCUMENT_TYPE, COLUMN_READ_NOT_IMPLEMENTED) as a STRUCTURAL gap, not TRANSIENT; a mixed genuine+structural failure stays on the transient path (next slot). Round 2: when every failure is structural but no gap key could be computed, the row takes the charged next-slot path and the log says so. Out of scope, still a timer: `filing-auto-persist.ts` extraction-failure backoff (`backoffNextDueAt`), which re-extracts a stored document, not a fetch — tracked as #959. |
...
29 rule(s) from `docs/design/rules.json`, generated by
`node docs/design/apply-rule-ownership.mjs --apply` from `rule-ownership.json`.

| Design section | Rule ids |
|---|---|
| §2.1 | R-003, R-004, R-005, R-006, R-007, R-010, R-011, R-012, R-013, R-014, R-015, R-016, R-181, R-182, R-183, R-184, R-185, R-187, R-188, R-227, R-228 |
| §2.1.3 | R-019, R-020 |
| §5.1 | R-102, R-103, R-104 |
| §7.4 | R-147, R-148, R-230 |
