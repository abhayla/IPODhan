...
- **F-101 (MAJOR) - an ALL-unresolved (ipo, table) pair is excused by `q_field_sources_row_key_coverage` as 'not row-keyed yet' instead of failing.** Found in the s7a review (2026-09-11), NOT closed here, by instruction. `classifyRowKeyCoverage` (scripts/lib/row-key-coverage-checks.mjs) skips a pair whose field_sources rows are all `''` (`hasAnyRealKey === false`), on the assumption that only a not-yet-live writer produces that shape. s7a adds a second producer of exactly that shape: when the consolidator is missing or throws, the helper returns for the WHOLE table at once, so every row of that pair is written with no per-row provenance and the pair reads as 'writer not live'. The check reports UNVERIFIABLE, not FAIL. A PARTIAL skip is caught (the resolved rows leave real keys, so the pair is enforced and the missing keys are named); the total case is the one that hides, and it is the likelier one because the fallbacks are per-table. Second, narrower hole: `rows.length <= 1` puts a single-row pair out of the class entirely, so a one-promoter IPO written unresolved is never judged. Until this is closed, the only surface for those rows is the NAMED `unresolved_child_rows` summary line and the '[FilingPersister] child rows written WITHOUT per-row provenance' log line s7a adds - a record that is honest but is not the record the checker reads.
