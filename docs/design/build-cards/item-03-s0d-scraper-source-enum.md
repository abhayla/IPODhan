...
| `scraper/tests/unit/pipeline-stages/stage-0-db-rebuild.test.ts` + `scraper/tests/unit/pipeline-stages/fixtures/stage-0/expected-schema.json` | another journal reader: pins `journalEntries` (a hard count, not derived) — this slice's JOURNALED migration bumps it 44→45; bump the fixture and its `_journalEntriesNote` in the same commit as the schema/migration change, per the note's own convention |
