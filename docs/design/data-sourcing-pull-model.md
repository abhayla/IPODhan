...
| **Opening-day check** | about **09:45**, only on a day an IPO is due to open (OD-31) | `heavy`, skipped if held | the two exchange lists only: register a new or changed IPO so the live jobs can see it | never fetch, download or extract a filing; it writes identity, status and the open and close dates, nothing else (OD-87); "a day an IPO is due to open" is read from the two fetched lists or the stored rows, so an IPO not yet stored is still caught |
