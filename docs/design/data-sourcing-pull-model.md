...
| OD-29 | *"Yes I accept 15-minute delayed prices from the exchanges' public endpoints, with the 90-day windows."* Measured 2026-09-24 (F-160): 121 of 129 IPOs listed in the last 90 days have no ISIN on staging, which the BSE half of this job needs; the pull walk mostly never asks for it. — no broker feed reaches the public site | 2026-09-09 | §2.1, §1.9 | a post-listing price window of 90 days at 15 minutes is stated, and no broker feed (Kite, SmartAPI, Upstox) is named as a public-site source (D17) |
| OD-30 | Within one document type the later filing date wins; a corrigendum overrides only the fields it names and freezes them; the final prospectus is terminal (corrigenda: after admin review, OD-90) | 2026-09-09 | §2.5, §3 | the three rules are stated and each names its test (D17) |
...
| OD-34 | Identity binds in order: CIN, then the SEBI draft filing number, then the exchange symbol, then the normalised name; a name-only row is flagged (step 2 replaced by OD-89) | 2026-09-09 | §2.3 | the four-step order is stated in that order and the name-bound flag exists (D17) |
...
| OD-56 | *"Go with your recommendation"* on the single-pass question, answering *"I don't want the scraper to run again and again for the same IPO... the first round of scraper itself is correct"* — the scraper reads an IPO **once per STAGE CHANGE**, never more: announced (DRHP: company, promoters, financials, objects), open (RHP + exchange: price band, lot size, dates, subscription), closed (allotment basis, final subscription), listed (listing price). **Within a stage a field that was read successfully is NEVER read again.** A field whose source did not yet exist at the previous stage is a FIRST read at this stage, not a re-read — the distinction that keeps "no second round" from meaning "a price band that is published after announcement is missed forever". Measured at the time of the decision: 13 UPCOMING IPOs held only 5 documents between them, so a literal one-scrape-ever rule would publish a blank price band for every IPO that files its RHP after announcement | 2026-09-18 | §2.1, §3 | no section schedules a repeat read of a field already SUPPLIED within the same stage, and every stage boundary names what becomes newly available at it |
| OD-57 | *"Go with your recommendation"* on the tie-break question — **the winner on a disagreement varies by field family, not one global order**: (a) **dates and status** (open, close, listing, allotment) → the **exchange** wins, because the offer document states an INTENDED schedule while NSE/BSE record what actually happened; (b) **money and share counts** (issue size, price band, face value, fresh issue, OFS, lot size) → the **document** wins, being the legally filed figure, and websites retype it (the BSE share-count-as-rupees class); (c) **names and identifiers** (registrar, lead managers, company name, symbol, ISIN) → the **document** wins AFTER normalisation, since "Mudra RTA Ventures Private Limited" vs "Mudra RTA Ventures Pvt Ltd" is a formatting difference, not a disagreement; (d) **live numbers** (subscription, GMP, current price) → the **exchange** always, the document cannot contain them. One sentence: **the document is the authority on what the company promised; the exchange is the authority on what actually happened.** Measured at the time of the decision: of 28,946 real disagreements, dates were the largest family at 12,719 across 44 IPOs, money/counts 8,938 across 229, names/ids 6,344 across 55 — so a global "document always wins" rule would have been wrong 12,719 times | 2026-09-18 | §2.3.5, §3 | the priority resolution is stated per field family, and no section applies a single global source order to both dates and money |
| OD-58 | *"Also increase the number of sources for better confirmation for correct data"* — a field is confirmed by AGREEMENT BETWEEN INDEPENDENT WITNESSES, so the witness list is widened beyond the four usable today (DOC, NSE, BSE, CHITTORGARH; MONEYCONTROL is retired per OD-3). At least one witness must be a source OUTSIDE the offer document — the data the exchange PUBLISHES on its public pages, not the document downloaded from it, because re-reading the same PDF proves only that the parser is consistent. Measured at the time of the decision: **all 8,010 field instances in `field_sources` had exactly ONE source** — no field had ever been written by two sources, so the cross-check had never once had the data to run; and of 190 manifest fields, 50 had three ranked sources, 72 had two, and **68 had exactly one** (document-only fields such as `cin`, `compliance_officer_phone`, `cut_off_price`, `sebi_regulation_cited`, which no exchange publishes) | 2026-09-18 | §1.1, §2.3.5 | every field names its witnesses; a field with two or more carries a comparison rule; the document-only fields are labelled as such rather than described as verified |
| OD-59 | *"Go with your recommendation"* on comparison semantics — **agreement is judged on the MEANING of a value, never on its text.** Numbers compare as numbers (`10`, `10.00` and `₹10` are one value); dates compare as dates; text compares after normalising case, punctuation and corporate-form words (`Pvt Ltd` = `Private Limited`, so "Mudra RTA Ventures Pvt Ltd" and "Mudra RTA Ventures Private Limited" are the SAME registrar, not a disagreement). **Money and share counts agree within 0.5%** — independent sources round the same figure differently — but **identifiers (ISIN, CIN, symbol, PAN) must match EXACTLY**, because there is no "close enough" for an identifier. Measured at the time of the decision, classifying all 28,946 recorded disagreements by shape: 19,967 genuinely different text, 3,595 genuinely different numbers, but **2,494 were the same number written differently, 1,207 were within 0.5% rounding, and 1,683 were a source returning zero** — so at least 5,384 of them (19%) were never disagreements at all, only comparison failures. Without this rule roughly one field in five is flagged DISPUTED for no real reason, the verdict column becomes noise, and a reader who sees "sources differ" on every second field stops trusting the warning — which defeats the purpose of having one | 2026-09-18 | §2.3.5, §3 | every field with two or more witnesses names its comparison rule by type, and no section compares two values as raw strings |
| OD-60 | *"Go with your recommendation"* on abstentions — **a source that returns zero, null or an empty value has ABSTAINED; it has not voted.** A field whose other witnesses agree but whose third witness returned nothing is `UNCONFIRMED` (only one real answer) or `CONFIRMED` (two real answers that agree), and is **never** `DISPUTED` on the strength of the silence. Treating an absent value as a contradicting opinion is what produced **1,683 false conflicts** in the measured data — `issueSize` of `"8250000000.00"` against `0` is not two sources disagreeing about the issue size, it is one source having no data, recorded as though it had an opinion. The distinction also protects a correct document value from being made to look doubtful by a website that simply had not published yet, which is the ordinary case for a price band between announcement and the RHP (OD-56) | 2026-09-18 | §2.3.5, §3 | the verdict rules distinguish "no answer" from "a different answer", and no section counts a null, zero or empty response as evidence of disagreement |
| OD-61 | *"keep everything admin only... no user should not see any disagreement"* — **a disagreement between sources is NEVER shown to a reader.** The page shows the winning value and, per OD-39, where it came from and when it was last confirmed; it shows no caveat, no "sources differ" marker and no second value, whatever the verdict. Disputes live only on the admin surface. This CONFIRMS the behaviour `web/components/ipo-detail/FieldProvenanceLine.tsx` already implements, and settles it as an owner decision rather than an implementer's reading: that component's header asserted "a disagreement between sources is never shown here. Per OD-39 that is admin-only", but OD-39 itself says only *"The reader should see where a number came from and when it was last confirmed"* and never mentions disagreements — so the rule was, until now, a code comment citing a decision that did not contain it. The owner also notes a future admin feature may let an admin see and hand-edit disputed fields on an IPO's page; not in scope now. Measured at the time of the decision: the admin surface for this ALREADY EXISTS and works — `/admin/conflicts` (view, filter by severity and IPO, one-click resolve with source selection, bulk resolve, auto-resolve, optional field protection, audit trail), `/admin/edit/[slug]`, and five `/api/admin/conflicts/*` routes — with 31,659 of 45,804 conflicts resolved, but **every resolution recorded `resolved_by = SYSTEM`** (31,653 `AUTO_RESOLVED_VALUES_CONVERGED`, 6 `EXCHANGE_CONSENSUS_OVERRIDE_HELD_VALUE`): no human has ever resolved one, and the 14,145 still open are precisely the cases automation could not settle | 2026-09-18 | §2.11, §3.4 | no reader-facing section renders a conflict, a verdict other than the chosen value, or a second source's value; §3.4's visibility for an unresolved disagreement is the nightly report and the admin surface, never the public page |
| OD-62 | *"Go with your recommendation"* on missing values — **a field no source could supply stores a REASON CODE, never a bare null.** Four codes cover the measured cases: `SOURCE_UNREACHABLE` (the site was down, refused, or the host was not resolvable), `NOT_PUBLISHED_YET` (the value does not exist yet — the ordinary case for a price band between announcement and the RHP, per OD-56), `EXTRACTION_FAILED` (we held the document but could not read the field), `FAILED_VALIDATION` (a value was read but failed its shape check, e.g. a 7-digit phone number or a CIN that is not 21 characters). A null teaches nobody anything and cannot be acted on; a reason code also lets the NEXT stage pass decide intelligently — `NOT_PUBLISHED_YET` means ask again at the next stage boundary, while `FAILED_VALIDATION` means the extractor itself needs fixing, and those deserve opposite treatment. Measured at the time of the decision: **12,701 plan rows across 71 IPOs currently hold no value** (NOT_AVAILABLE_YET 6,437 / 60 IPOs, CHECK_FAILED 6,232 / 71, EXHAUSTED 32 / 32), and `field_extraction_failures` — the table item 4 built to carry exactly this — **holds ZERO rows**; working out that the underlying cause of one 4,189-row cluster was missing `company_website` values took an afternoon of ad-hoc querying that a reason code would have answered in one **Extended 2026-09-23 by OD-77 with a fifth code, `NOT_SOURCED`.** | 2026-09-18 | §2.5, §3 | every terminal not-supplied state carries a reason code from the named set, and no section leaves a field absent without one |
| OD-63 | *"Also add them to the admin/conflicts so that admin can review and update"* — **the admin surface shows ABSENCES as well as DISAGREEMENTS.** `/admin/conflicts` is extended from a disagreement queue into a data-quality queue: alongside each unresolved conflict it lists every field carrying an OD-62 reason code, grouped by reason and by IPO, and lets the admin supply the value by hand through the existing field-edit and field-protection path. Rationale: a disagreement and an absence are the same problem for an admin — a field whose value cannot be trusted — and splitting them across two screens means the larger population is the one nobody looks at. Measured at the time of the decision: the dashboard already exists and works (view, filter by severity and IPO, one-click resolve with source selection, bulk resolve, auto-resolve, optional field protection, audit trail, plus `/admin/edit/[slug]` and five `/api/admin/conflicts/*` routes), and 31,659 of 45,804 conflicts are resolved — but **every resolution records `resolved_by = SYSTEM`, so no human has ever used it**, while the absence population it does not yet show is 12,701 fields across 71 IPOs | 2026-09-18 | §2.11, §3.4 | the admin surface section lists both the unresolved-disagreement queue and the reason-coded absence queue, and names the path by which an admin supplies a value by hand |
| OD-64 | *"For IPOs which are only for NSE... NSE should be first source... BSE should not be there because this is an NSE only IPO. If there is a BSE only IPO again the status should come from BSE... but NSE should not be an option"* — **source priority for `status` and the price band is decided by WHERE THE IPO IS LISTED, not by a fixed global order.** The listing venue is the authority on its own IPO's status and band; the other exchange has no standing to speak about an issue it does not list. Ranking: **NSE-only** -> NSE, then other websites, BSE NOT ELIGIBLE. **BSE-only** -> BSE, then other websites, NSE NOT ELIGIBLE. **Both** -> the two exchanges first (implementer picks the order between them), then other websites. The rule is about ELIGIBILITY as much as order: an exchange that does not list the IPO is not a lower-ranked source, it is not a source at all. Measured at the time of the decision: `ipos.listing_exchanges` (jsonb `('NSE'\|'BSE')[]`) holds BSE-only 193, BOTH 105, NSE-only 71; the matrix today ranks `status` as `['ADMIN','NSE','BSE','MONEYCONTROL','CHITTORGARH']` and `priceRangeMin/Max` as `['ADMIN','DRHP','NSE','BSE','MONEYCONTROL']` — **fixed orders that never read `listing_exchanges`**, so NSE outranks BSE even on a BSE-only IPO. Today ZERO rows actually violate the rule (every BSE-only IPO is BSE-sourced, every NSE-only NSE-sourced) because each scraper only reads its own board — the data is clean by accident of collection, not by design, and nothing prevents the cross-write. **Note (F-135, measured 2026-09-23): NSE IPO itself is a case where the price band ad and RHP p.3 say listing is on BSE only, while staging `listing_exchanges` holds `["BSE","NSE"]` because NSE ran the bidding — a source of listing_exchanges pollution distinct from this OD's ranking fix; issue #938; no rule changed here.** | 2026-09-21 | §2.2, §2.4 | the priority matrix resolves `status` and `priceRangeMin`/`priceRangeMax` through the IPO's `listing_exchanges`, and a non-listing exchange is excluded rather than demoted |
| OD-65 | *"Everything should be done on day one itself, and it should not be repeated for the same IPO. I don't care what happens in Chittorgarh or any other website after that... one IPO, one round of document read"* — **a website's later disagreement with an offer document is NOT re-examined.** The comparison between the document and the websites happens when the document is read, and once. A website that changes its mind on day 4 does not reopen a settled field, and no job goes back to a document to re-adjudicate. This supersedes §3.2's re-read loop as a standing behaviour (see OD-67). Measured: the re-read loop was sized from "578 leadManagers conflicts"; after removing rows where a source disagrees with ITSELF, rows with a null on one side, and byte-identical values, the real count is **1 genuine conflict on staging and 2 on prod** | 2026-09-21 | §3, §3.2 | no section schedules a re-comparison of a settled field against a website, and no job re-reads a document to resolve a website disagreement |
...
| OD-87 | *"Identity, status + dates (Recommended)"* — 2026-09-24, spec-verified first (Spec basis: §2.1 opening-day row, the OD-31 subsection, §7.4). **The opening-day check writes identity, status, and the open and close dates — nothing else.** Status is derived from those two dates, so an IPO first seen at 09:45 with no dates could not show as OPEN and the live jobs would skip it until 14:00. Still never price band, lot size, issue size, subscription or any document. The check also no longer depends on a stored row: it reads the two exchange lists and acts on any IPO whose listed or stored open date is today (IST), because the IPO it exists for is the one not yet stored. Real case: NSE list 2026-09-24 — Moneyview Limited, A-One Steels India, Green Asia Impex, each open 24-Sep, close 28-Sep. | 2026-09-24 | §2.1 | §2.1's opening-day row names identity, status and open/close dates, and says the day is decided from the fetched lists or stored rows |
| OD-88 | *"(a) NSE creates (Recommended)"* — 2026-09-24, spec-verified first (Spec basis: §2.1 opening-day row, OD-87, OD-85, #860). **The opening-day check creates a new IPO row only from the NSE list, which states the segment. An IPO on the BSE list that is not on the NSE list and not stored is NOT created: it is logged by name with the reason and left to the 14:00 data job.** Rows already stored are updated from either list (status and the open and close dates, OD-87). Why: an IPO is never created without a segment (#860 — the segment decides which manifest ranks its fields get), and the BSE board row carries no segment. Consequence, not a new rule: an NSE row whose list states no segment is likewise not created. Every column the check writes carries its `field_sources` row. Real cases: BSE board 2026-08-27 — Kwick Forensic Solutions Limited (IPO_NO 7913) and Lumino Industries Limited (IPO_NO 7910) opened on the BSE list only; NSE list 2026-08-21 — Augmont Enterprises Limited (series EQ, MAINBOARD). | 2026-09-24 | §2.1 | §2.1's opening-day row says new rows come only from the NSE list; `scraper/tests/integration/opening-day-create.integration.test.ts` creates the NSE row through the real `IPORepository.create` with one `field_sources` row per column, and a BSE-only newcomer is not created and is logged by name |
| OD-89 | *"(a) Drop step 2 (Recommended)"* — 2026-09-24, spec-verified first (Spec basis: OD-34, §2.3 binding order table, F-125, OD-85). **Step 2 of OD-34 ("the SEBI draft filing number") is dropped. The identity binding order is: CIN, then each source's own record number (OD-85: BSE IPO number, NSE issue, Chittorgarh page id), then the exchange symbol, then the normalised name.** Why: SEBI numbers each document separately and nothing links a draft's number to its RHP's (F-125, e.g. DRHP 102189, RHP 104428, Prospectus 104637); the only per-offering number, the observation-letter number, is printed only in the RHP/Prospectus (filled in 10 of 12) and unfilled in 6 of 7 DRHPs, by which time CIN is usually known. No column, no migration and no extractor are added for it. | 2026-09-24 | §2.3 | the §2.3 binding table lists CIN, source record keys, symbol, name, and F-125 is marked resolved by OD-89 |
| OD-90 | *"(a) Admin review queue (Recommended)"* — 2026-09-24, spec-verified first (Spec basis: OD-30, section 2.5.5, E-1, section 2.2.1, F-163). **SPEC CHANGE to OD-30: a corrigendum is applied after ADMIN review, not automatically.** The pipeline reads each stored corrigendum (text layer, else OCR, marked) and records a SUGGESTION per changed field: the field, the stored value, the proposed value, the quoted sentence and its page. An admin accepts or dismisses each suggestion; an accepted one is written as an ADMIN value, which already outranks every source and holds, so no separate freeze column is needed. An exchange-owned field (E-1, e.g. close_date) shows the exchange value beside the suggestion, and nothing is written without the admin. Why: of three real corrigenda (Rays of Belief, Skyways, Hy-Tech) only one changes a field a document may write, and the three use unrelated phrasings (F-163). | 2026-09-24 | section 2.5.5 | section 2.5.5 states that corrigenda produce admin-reviewed suggestions (field, old, new, quote, page) and that an accepted one is an ADMIN write |
| OD-91 | *"Going forward"* — 2026-09-24, spec-verified first (Spec basis: section 2.5, section 2.5.5, OD-65, OD-73, F-164). **Each document's extraction writes a per-document record of the fields it produced (`document_field_receipts`), in the same transaction that marks the document COMPLETED. Supersession (section 2.5) reopens a plan row only when the newer, higher-ranked document's own record has that field, and the DOC fetcher credits the best-ranked document whose record has the field. The 28 existing staging rows (F-164) keep their value and chosen document, and no document is read again (OD-65 kept); no backfill re-extraction.** A record is not provenance: it does not re-stamp `field_sources`, so OD-73 is unchanged. **Round-2 details (2026-09-24, owner-delegated, spec-conformant):** (1) the per-field document-family table is one config file (`scraper/config/document-families.json`), and one rule module (`scraper/config/plan-supersession-rule.mjs`) is run by both the scraper and the PULL-FROZEN audit; (2) each record stores the normalised value the document extracted, and the DOC fetcher credits a recorded document only when that value equals the column's current value (OD-73 "identical"); otherwise the current choice stands and the mismatch is logged with ids; (3) the fixed-price test (section 2.5.5 Rule 3) reads `ipo_details.issue_type` for every table's field, the same on the write path and in the fetcher; (4) `ipos.offering_type` and `ipos.segment` are copied from the stored row as identity context and `ipos.lead_managers` is read from the stored row, so no document record covers them. | 2026-09-24 | section 2.5 | section 2.5 names the per-document record as the input to supersession and to the DOC fetcher's choice, and states that documents extracted before it keep their rows |
| OD-92 | *"owner delegation 2026-09-24: go with the recommendation if it conforms to the spec"* — 2026-09-24 (item 19; Spec basis: section 2.3.3.3 "a merge log, not a diff ... the rows themselves" and "every automatic merge is reversible", OD-38, OD-86). **The merge log stores, whole and inside the merge transaction, every row the merge deletes: the direct scraper-derived children of the removed IPO AND every row their foreign-key cascades remove, plus every reference a SET NULL cascade clears and the prior state of any source key an OD-86 relaunch merge supersedes. The unmerge restores all of it. Entries logged before this change are partly reversible, and the tool says what it cannot restore and refuses unless told `--partial`.** Measured before recording: the one staging merge (rays-of-belief-ltd-o into rays-of-belief-ltd, 2026-09-24) logged 540 deleted rows in 13 tables as counts only, including 119 gmp_records and 2 documents. Rules of the unmerge, recorded here under the same delegation (inferred, not spec-stated; they keep "restores both rows" from undoing work done after the merge): (1) on the survivor it restores ONLY the columns the merge carried, and the provenance rows of those fields, to their pre-merge values; every other survivor column and every other provenance row keeps its current value, and updated_at goes back only when nothing wrote the row after the merge; (2) it refuses when a carried column or its provenance row changed after the merge, naming them, unless each is named in `--force-fields`, which accepts carried columns only; (3) before any write it checks every row it would restore against every unique index (read from the catalog), and on any clash it prints "refused: <table> <constraint> collides with survivor row <id>" for each, writes nothing, and says to resolve or remove the named rows and re-run; (4) logged values are restored inside the database, so bigints above 2^53 and numeric scale come back exactly, and both ipos rows are read back against the log, rolling back on any mismatch. Wording fix in the same row: the tool path in section 2.3.3.3 is `scraper/scripts/repair-merge-duplicate-ipo.ts` (`--unmerge <merge-id>`); `scripts/merge-duplicate-ipo.mjs` does not exist. | 2026-09-24 | §2.3.3.3 | a merge then unmerge on ipodhan_test leaves every affected table row-for-row equal to the pre-merge snapshot, including an OD-86 relaunch merge (scraper/tests/integration/merge-unmerge-roundtrip.integration.test.ts) |
| OD-93 | *"go with the recommendation if it conforms to the spec"* — owner delegation 2026-09-24, applied to the item 21 read-side choices that OD-39, OD-40 and OD-72 leave open. Spec basis: OD-39 (the source and when it was last confirmed), OD-40 (one authenticated call per cycle with the touched slugs), OD-72 (source and READ date; the time of a live figure; a missed OD-19 slot alerts the admin), OD-19 and section 2.1 (the live-figures job every 30 minutes, 10:00-18:30 IST, on a bidding day), the IST rule. Five choices, each conforming: (1) **the read date** is a new plan-row column `chosen_confirmed_at`, stamped when a row is recorded SUPPLIED with its evidence; `updated_at` moves on any write and is not that date; rows already SUPPLIED with a chosen source when the column was added DO have a recorded read time (`last_attempt_at`, stamped in the same write as the SUPPLIED answer) and are backfilled from it in the same migration, 236 rows on staging; a SUPPLIED row that names no source gets no date; a key-facts block shows a date only when every field in it has one; (2) **the wording** is "From <source>, read <d Mon yyyy>" in IST, and a live figure reads "<figure> as at <h:mm AM/PM>, <d Mon>" from the source's own observation time; (3) **the cache key** of the provenance line is its own slug-keyed key `ipo:fieldplan:provenance:<slug>`, dropped by the OD-40 call with the page's key (section 2.11 had placed the line inside the `getIPOBySlugKey` payload, which the built page does not do); (4) **a missed live slot** is one of the 17 half-hour slots from 10:00 to 18:30 IST on a weekday that is not a market holiday, for an IPO whose open and close dates span that day, in which no subscription figure dated by the source falls; every live wake, including one that hit its deadline, judges EVERY ended slot of the IST day, so skipped wakes are still judged; an IPO gets ONE admin alert per IST day, at its first missed slot (dedupeKey `live-slot-miss:<env>:<ipo>:<day>`), and one end-of-day summary lists each IPO with all its missed slots; titles name the environment (staging or prod), severity P2; a claim is written only after the Notifier accepted the alert, and a failed or unconfigured send is logged at warn with its reason and retried by the next wake; a live cron dead for the WHOLE day is outside this check, and the data job's freshness SLO `open-ipo-gmp-subscription` is what sees it (the nightly detection floor has no subscription-freshness check); (5) **the touched slugs** are mirrored into one Redis set, read and cleared atomically (RENAME, then read), so a crash between a write and the end-of-cycle call is revalidated by the next cycle's one call. Measured on staging 2026-09-24 before building: 236 SUPPLIED plan rows, none with a stored read date; for the 14:00 IST slot, 9 of 19 bidding IPOs had no figure inside the slot (anand-seamless-ltd, s-k-offset-ltd and 7 more) while 10 did; over 15-23 Sep the per-IPO rule sends 4 to 18 alerts a weekday (38 to 170 IPO-slot misses a day); on 23 Sep every bidding IPO missed the 17:00 and 18:00 slots. (If a parallel change takes this id first, this row is renumbered.) | 2026-09-24 | §2.11 | section 2.11 names `chosen_confirmed_at` as the read date, the slug-keyed provenance cache key dropped by the OD-40 call, the live-figure time line, the missed-slot admin alert rule, and the Redis-backed touched slugs |
| OD-94 | *"owner delegation 2026-09-24: go with the recommendation if it conforms to the spec"* — 2026-09-24 (item 12, #679; Spec basis: OD-69 "the merge tool refuses a pair whose CIN, symbol, ISIN or open date differ", OD-70 "a listed company's later event ... is its own row of its own type", OD-86 "a merge of two rows whose open dates differ ONLY for an OD-83 relaunch", OD-38, §1.11 offering types, §2.3.3.2, §2.3.3.3). **The merge tool also compares offering_type, close_date and listing_date. It refuses a pair whose offering types are both populated and differ, in every case, relaunch included, because two offering types are two offers. It refuses a pair whose close dates, or whose listing dates, are both populated and differ, unless the OD-86 relaunch test passes, because a relaunch moves every date. An empty side is not a disagreement. Every refusal names the column and both values in the tool output.** Measured before recording: on ipodhan_staging the pair cube-highways-trust (IPO, 2026-07-19 / 07-26 / 07-29) and cube-highways-trust-cube-highways-trust-invit (INVITS, 2026-07-22 / 07-24 / 08-03) was accepted in both directions (#679), and the IPO row is already gone on staging: its slug redirects to the INVITS row with reason DUPLICATE_MERGE and no ipo_merge_log entry. The Rays of Belief merge (same type, same three dates) and the Dhanwel relaunch (F-144, dates moved by the exchange) still pass. (Numbered OD-94 because OD-91 is reserved by open PR #992 and OD-93 is on main; renumber if either moves.) | 2026-09-24 | §2.3.3.2, §2.3.3.3 | scraper/tests/unit/duplicate-ipo-merge.test.ts "offering type and close/listing dates (#679, OD-94)" pins the Cube Highways refusal both ways, the Rays of Belief acceptance and the Dhanwel relaunch acceptance |
| OD-95 | *"owner delegation 2026-09-24: go with the recommendation if it conforms to the spec"* — 2026-09-24 (#968; Spec basis: section 2.3.5 "takes effect at the next cycle" and "effective order ... recorded on the plan row", OD-73 "after that only a HIGHER-ranked source may change it", OD-65, section 2.5.1, OD-91). **Under an active override, "higher-ranked" (OD-73) means higher in the override's effective order. A settled (SUPPLIED) plan row reopens only when (1) the incoming order comes from an override, (2) its policy_origin differs from the one the row recorded, and (3) it ranks a source above the settling source that was not already above it in the row's recorded order. A plain re-plan, including a registry version bump, reopens no SUPPLIED row. The reopen is stored on the row (`ipo_field_plan.reopened_under_policy`), so every later plan pass and walk honours it: the walk asks only the sources ranked above the settling source, and a source at or below it is not asked, so it cannot overwrite the settled value. The value on the page stays until a higher source answers. If no higher source has the field (every one gives a definitive no), or the override expires or is replaced by one that no longer outranks the settling source before a higher source answered, the row returns to SUPPLIED with its previous value and evidence. A row re-supplied under the override keeps the new value after the override expires, because it came from a higher-ranked source at the time it was written.** Why: the first attempt (#967) reopened on "chosen source is rank 2 or 3", which is true on every re-plan with no override, so 24 staging SUPPLIED rows would have reopened every data slot; and its "ask only above rank N" narrowing lasted one pass. Measured read-only on ipodhan_staging before building: 236 SUPPLIED rows, 0 active overrides; the #967 condition selects 24 of them, this rule 0; re-applying the Swap Test override (ipos.issue_size CHITTORGARH over DOC) reopens exactly 2 (lcc-projects-ltd, rentomojo-ltd, both settled by DOC) and leaves the 17 CHITTORGARH-settled rows alone. An ADMIN value (section 2.7) is not reopened by an override. **Fix round 1 (Tier A review of #1005, same delegation):** (a) a row is reopened at most ONCE per distinct override: when a reopen ends without a new value, the row records the order it was tried under (the override's policy_origin and its ranks), so the next plan pass under the same override changes nothing; only a different effective order can reopen it again. (b) A higher source that answers but loses to the field-priority matrix counts as a definitive no for that override: the row is restored (tried once) and the refused value is recorded in the admin conflicts list under the named reason OVERRIDE_SOURCE_LOST_TO_PRIORITY (admin-only, not an alert; the OD-61 and OD-75 shape). (c) Before a reopened row is restored, the OD-91 supersession rule is run for it against the IPO's completed documents; if a better document with this field completed during the reopen, the row is handed to supersession (PENDING with superseded_by) instead of the outranked evidence being restored. (d) When the override ends, the plan pass makes the row due and the walk performs the restore, so (a) and (c) apply to every restore. (e) Migration 0061 (reopened_under_policy) is applied before this code runs; the deploy applies migrations before starting the new release. | 2026-09-24 | §2.3.5, §3.2 | section 2.3.5 states the override reopen rule, the stored narrowing and the expiry rule; scraper/tests/unit/services/settled-field-override-reopen.test.ts and scraper/tests/integration/settled-field-override-reopen.integration.test.ts pin no-override-reopens-nothing, narrowing-survives-two-passes, lower-rank-never-overwrites and override-expiry-no-flip-flop |
| OD-96 | *"owner delegation 2026-09-24: go with the recommendation if it conforms to the spec"* — 2026-09-25 (#993 round 1; Spec basis: §1 `DOC` = "the IPO's own offer document, best available type", row 20 `company_description` and row 32 `cin` rank DOC first, row 10 `registrar_id` is class C "derived; never sourced", OD-7 nothing is blanked, OD-91 document families). **A document writes a field only when that field's manifest document family contains the document's type.** The family is the one the DOC fetcher reads the field back with (`scraper/config/document-families.json` through `plan-supersession-rule.mjs` `familyForField`, no copy); a field with no manifest documentType keeps that rule's meaning (the writing document's own type is its family). A refused column is not written by that document and its stored value stays (OD-7); the persister lists it in `skipped_out_of_family`. Why: once `ipos` provenance names its document (#993), a price-band advertisement writing an RHP-family field (`ipos.company_description`, `ipos.cin`) leaves a row the fetcher refuses to credit (CHECK_FAILED) and OD-73 never lets it recover. Measured read-only on ipodhan_staging 2026-09-25: every out-of-family pair is PRICE_BAND_AD writing an RHP-family field — ipos.company_description 3 rows / 3 IPOs, ipo_details.compliance_officer 21, compliance_officer_email 16, company_description 12, pre_ipo_placement 12, compliance_officer_phone 11, ipo_intermediaries name/role 148 rows / 11 IPOs, promoters name 16 / 5, financial_statements eps 16, financial_data 11; each of the 26 IPOs with a completed advertisement also has a completed RHP, DRHP or PROSPECTUS, so no value goes unfilled. `ipos.registrar_id` is derived inside `upsertIPO` (`scraper/src/services/data-persister.ts` `resolveRegistrarIdSafe(sanitizeRegistrar(scrapedIPO.registrar))`), not sourced; it has no manifest entry. **Applied to `ipos` in #993; extending it to the child tables reverses the DEEPA-walk rows W-76 and W-88 (the advertisement writes syndicate members, banks and the compliance officer) and is open for owner decision.** | 2026-09-25 | §2.5 | scraper/tests/unit/services/filing-persister.test.ts "OD-96: a price-band advertisement does not write ipos.cin"; scraper/tests/unit/services/document-family-gate.test.ts |
| OD-97 | *"owner delegation 2026-09-24: go with the recommendation if it conforms to the spec"* — 2026-09-25 (item 22; Spec basis: OD-36 and section 2.2.1 "Image-only pages go to OCR, marked": "a field whose only source is an OCR page never wins a disagreement against a text page"; OD-73; OD-65 and OD-91, no re-read; F-167). **The OCR mark travels with each value, and an OCR-only value loses a disagreement to a text-page value.** (a) The extractor envelope lists `ocr_pages`; a receipted value is marked `TEXT` (every page behind it has a text layer), `OCR` (every page behind it was OCR'd) or `MIXED` (some of each, or no page named inside an OCR'd document). The mark is stored in `document_field_receipts.source_text` and `ocr_confidence` (the lowest OCR page confidence), and in `field_sources.data_lineage.ocr` wherever the filing persister writes provenance. (b) Values written before this change carry no mark and count as unknown; the rule fires only when the incoming side is known `OCR` and the stored value is one a known `TEXT` read of the same IPO produced from a document of the same or better rank for the field (see (d)), and it does not fire on `MIXED` or unknown. (c) In a disagreement between two offer-document values for the same field, the known OCR-only value loses to the known text-page value, whatever the document order: an OCR value is not written over a text-supported stored value, and a text value is written over an OCR one as before. It decides between candidate values only and does not reopen a settled field by itself (OD-73). If the stored side, the text reads or the OCR value's own document cannot be read, the OCR-only value is held back. (d) **Rank scope** (supervisor decision under the owner delegation 2026-09-24, #1012 fix round 1, Tier A review MAJOR-1): a text read outvotes an OCR-only value only when it was read from a document of the SAME OR BETTER rank for that field than the OCR value's own document, ranked by the one plan-row supersession comparator (section 2.5; OD-30: within a type the later filing wins and the final prospectus is terminal; across types the precedence table; section 1 `DOC` = "the IPO's own offer document, best available type"; the field's family is its manifest documentType's family). A text read of an older or lower-ranked document never beats a better document: a DRHP text read does not outvote an RHP OCR value, while an RHP text read does outvote a DRHP OCR value. A same-type pair with a missing filing_date is unordered, so the text read counts and the stored value is kept. (e) An OCR-only value's provenance confidence is its lowest OCR page confidence on the 0-100 scale, not a flat 100. It covers the receipted scalar columns of `ipos` and `ipo_details`; whole-row tables keep a document-level mark only. | 2026-09-25 | section 2.2.1 | section 2.2.1's OCR row states the per-value mark, where it is stored, the unknown rule and the OCR-loses rule, and cites F-167 |
| OD-98 | *"owner delegation 2026-09-24: go with the recommendation if it conforms to the spec"* — 2026-09-25 (F-158/OD-90 follow-up; Spec basis: OD-90 "the pipeline reads each stored corrigendum", OD-56 "a LISTED document is attempted once, and again only for a new document", OD-65 "no re-read of an already-read document"). **The EXTRACTION pass (PASS 2) picks up a stored, never-read document (`extraction_status` PENDING, type extractable, `purged_unread`=false, file present) even when its IPO is OUTSIDE the live window `loadCandidateIpos` gates discovery/fetching on. Discovery and fetching stay live-window only — nothing else changes.** A document already sitting on disk with nothing left to fetch must not stay stranded once its IPO leaves the live window: skyways-air-services-ltd is LISTED with `listing_date` 2026-09-01 (24 days before this decision), and `LIVE_WINDOW_DAYS_AFTER_LISTING` is 10, so `loadCandidateIpos` never visits it again — re-admitting its corrigendum (id `3b77bc3b-e44a-4d3d-9622-edb71763e9e8`) to PENDING (the readmit-stranded-corrigenda repair) would otherwise leave it PENDING forever. Measured on staging: the stored file exists (`/var/www/ipodhan/shared/prospectus/staging/8340f7fc-.../CORRIGENDUM-d3a6094a.pdf`, 1,391,575 B, sha matches, `purged_unread`=false). Built at class level: `selectExtractionOnlyCandidates` (`scraper/src/services/document-cycle.ts`) reads the SAME `isExtractableDocType` predicate OD-90/#989 widened, additive to the live-window candidate set (never a double dispatch), so any IPO holding such a document — not only Skyways — is covered. | 2026-09-25 | §2.5.5 | scraper/tests/unit/services/document-cycle-extraction-only-candidates.test.ts (a LISTED IPO past its live window holding a PENDING stored CORRIGENDUM is a candidate; the same IPO with no such document, or with only a PENDING ADDENDUM, is not; the live-window gate `isInLiveWindow` that governs discovery independently excludes it) |
| OD-99 | *"owner delegation 2026-09-24: go with the recommendation if it conforms to the spec"* — 2026-09-25 (structural write refusals; Spec basis: OD-62 a reason code, never a silent null; OD-78 same cause, same outcome, no retry; #884/#923 gap codes, a configuration gap is not a failed attempt and is re-asked when its gap key changes). **A write the consolidated writer refuses for a STRUCTURAL reason is recorded, never silently re-queued.** A structural refusal (`MISSING_ROW_KEY`, `CHILD_TABLE_CONSOLIDATION_DISABLED`, `CONSOLIDATION_DISABLED`: the writer will refuse the same way every time it is asked) settles the plan row `CHECK_FAILED` with reason code `COVERAGE_GAP`, gap code `WRITER_CANNOT_ACCEPT`, the skip reason in `cause` and `last_attempt_at` set. Like every configuration gap it is NOT charged as an attempt (#923), so it can never reach the attempts cap and be locked out of reopening. It is asked again only when its gap key changes, and that key carries the writer's capability for the table (the consolidation flags, whether the table is keyable as one row per IPO, a writer version bumped by a code fix), so a writer fix reopens it; when no per-field key exists that cycle it is parked under the writer capability alone. A transient drop (a lost lock, an error mid-write) keeps the PENDING re-queue of section 2.3. Measured before the fix on staging 2026-09-25: 26 `gmp_records.gmp` rows looped on every wake as `MISSING_ROW_KEY` (failure class `dropped-write-strands-a-plan-row-pending`); after it they read `CHECK_FAILED` `WRITER_CANNOT_ACCEPT`, and the GMP job (section 2.1) keeps `gmp_records` itself fresh. | 2026-09-25 | section 2.3 | section 2.3's write-result paragraph states the structural refusal, the uncharged writer gap key and the transient re-queue, and cites OD-99 |
| OD-101 | *"Add PARTIAL shape (Recommended)"* — 2026-09-25 (#1027, item 32; Spec basis: §7.1 item 32, OD-52 §8.5 "every build card lists the ids it implements ... CI refuses a PR that breaks the chain"; the item-32 build card's own "The line's two accepted shapes, and nothing else"; the 2026-09-25 contract's item 32 step "Resolve each of the 10 unknown cards to DONE or NOT STARTED ... then make check-build-cards.mjs refuse unknown outright"). **A build card's `Status:` line gains a third accepted shape, `Status: PARTIAL <date> PRs #… proof … parked #NNN[, #NNN]`, for an item whose built part is proven and whose remainder is genuinely parked.** It is accepted only when every named issue is open and labelled `parked`, checked via one `gh issue view <n> --json state,labels` call per distinct issue number for the whole run (never per card), injected into a pure predicate so `scripts/tests/check-build-cards-status.test.mjs` needs no network for it. An unreachable `gh` fails the card closed; the local-only `--offline-parked-check=skip` flag is refused whenever `process.env.CI` is set. `REFUSE_UNKNOWN` is added as the constant the supervisor flips, once every `UNKNOWN_ALLOWED` card resolves to DONE / NOT STARTED / PARTIAL, to refuse `unknown` outright and let `UNKNOWN_ALLOWED` be emptied — not flipped in this change, because six cards (items 6, 7, 9, 19, 21, 22) still read `unknown`. Options (b) (write DONE and bury the parked remainder in the card body) and (c) (keep `unknown` and the allow-list until every parked piece closes) were rejected: reasoning in issue #1027. | 2026-09-25 | §8.5 | `docs/design/check-build-cards.mjs`'s `PARTIAL_RE` accepts the shape only when `validateStatusLine` resolves every named parked issue to open + `parked`; `scripts/tests/check-build-cards-status.test.mjs` pins acceptance, a closed issue, a missing `parked` label, an unresolved issue, `gh` unreachable (closed and, under the flag, open), the CI refusal of the flag, and the `REFUSE_UNKNOWN` flip |
| OD-100 | *"Remove from walk (Recommended)"* — 2026-09-25 (#1022; review round 2 MAJOR-1 generalizes the rule; Spec basis: §2.1 job table — GMP job "writes `gmp_records` only"; Live-figures job "subscription and the demand graph ... never touch ... a field plan row"; Post-listing price job "`ipos.current_price` and its as-of stamp ... never touches a document or a static field"; OD-73 "Live figures ... are never settled"). **A live figure with its own scheduled job in §2.1's job table is owned by that job, not by the field-plan walk — it gets no plan row at all, whatever rank the field-source table states.** The rule is driven from ONE declared list (`docs/design/field-source-resolution.spec.mjs`'s `JOB_OWNED_TABLES` for a table a job owns wholesale, `JOB_OWNED_FIELDS` for a job that owns specific columns of a table it does not own wholesale), read by `add()`'s generic `jobOwned` opt — the manifest generator excludes any field so marked, generically, never a per-table special case. Today that list is: **`gmp_records`** (whole table — the GMP job); **`subscriptions`** and **`ipo_demand_graph`** (whole tables — the Live-figures job, same job, same wake). **Not** `listing_performance.current_price*`: the Post-listing price job writes only `ipos.current_price` (`scraper/src/scheduler/post-listing-price.ts`), which is not a field-plan field, and no scheduled job writes the `listing_performance` quote columns, so they stay walk-owned (supervisor correction 2026-09-25 of review round 2; `JOB_OWNED_FIELDS` is empty today). The INVESTORGAIN_GMP field-plan fetcher (`field-plan-walk-investorgain-gmp-fetcher.ts`, item 6's stage-2 adapter) is retired along with its registration — subscriptions/demand-graph already had NSE/BSE walk fetchers registered, which is exactly why their rows were looping CHECK_FAILED (the same MISSING_ROW_KEY-shaped structural refusal class as gmp_records, OD-99) instead of never being asked. Existing rows are not left to linger: `scripts/repair-retire-manifest-removed-fields.ts` deletes every `ipo_field_plan` row whose `(table_name, field_name)` has left the manifest (snapshot + delete in one transaction, by the exact row ids snapshotted; ledgered, undoable). Measured on staging 2026-09-25 (dry run): 1,604 rows found and slated for retirement across the now-job-owned field keys (re-measured after the correction; see the PR body), 0 rows for any other field key. Item 6's "GMP walk proof" piece is PARKED-OWNER no more — it is dropped as not applicable. | 2026-09-25 | §2.1, §1 | §2.1's job table and §1's job-owned rows state the rule and cite OD-100; item 6's row states the proof is not applicable |
...
| OD-75 | *"Yes, own reason"* — follow-up to OD-73, chosen 2026-09-23 with the data: zero cases in 30 days on staging of a source changing its own earlier value, against 24,008 admin-conflict rows in the same 30 days (why T-286 banned same-source rows). **When a website changes a value IT set earlier, the page keeps the old value (OD-73) and the change is recorded in the admin conflicts list under its own named reason (a source changed its own value), visible to the admin, never to readers (OD-61), and never sent as an alert (T-286's flood protection stays)** | 2026-09-23 | §3.2 | the self-change rule, its named reason and the no-alert rule are stated |
| OD-76 | *"Plan, then walk"* — chosen 2026-09-23 after the first real staging cycle of the 22:00 job and three failed reviews of an off-spec fix (#912, which added a separate document-reading path §6.1 forbids). Measured: 238 of 274 LISTED IPOs on staging have NO field plan (plans exist only for IPOs live after the generator was switched on, ~2026-09-16); all 10 IPOs the job picked (e.g. riyaasat-lifestyle-ltd, advit-jewels-ltd, turtlemint-fintech-solutions-ltd) had 0 plan rows and 1-2 unread offer documents, so the walk had nothing to do and each was recorded DONE. **For each IPO it picks, the job first runs the EXISTING plan generator (creating that IPO's plan rows exactly as for a live IPO), then the same §2.4 walk, which uses the values already extracted from the IPO's offer documents (reading a still-PENDING document stays the document cycle's job, OD-33; that cycle is not limited by IPO status). One code path. (Corrected 2026-09-23: the first wording said the walk reads documents itself; it does not.) An IPO whose plan could not be generated (or has no rows) is never recorded DONE; an IPO whose walk asked nothing is DONE only when every one of its plan rows is already settled (OD-73), never when fields were still due.** | 2026-09-23 | §6.1 | §6.1 states generate-then-walk and the never-DONE-without-a-walk rule; no section describes a separate document path for closed IPOs |
| OD-77 | *"Add NOT_SOURCED"* — chosen 2026-09-23, spec-verified first: 49 staging rows store issue size = 0. §1.11 already settles 17 of them — TENDER (16) and BUYBACK (1) are "corporate actions, not offerings", so issue size is `NOT_APPLICABLE`, derived from `offering_type` and never stored (§2.7); their 0 is removed. For the other 32 — OFS 19 (e.g. coal-india-ltd, nhpc-ltd), RIGHTS 6, NCD 5, IPO 2 (advenzymes, light-of-life-trust) — §1.11 does not list issue size as not-applicable, so the 0 is a MISSING value, and none of OD-62's four codes is true for it. **OD-62 gains a fifth reason code, `NOT_SOURCED`: no source we read supplies this field for this offering. The 32 zeros are replaced with it (staging now; production with the release) and the page shows the field as not available. Sourcing the real amounts from the rank-1 sources §1.11 names (letter of offer, NCD prospectus, the exchange's OFS notice) is separate, later work.** | 2026-09-23 | §1.11, §5.3 | OD-62 lists five codes; §1.11's TENDER/BUYBACK row states issue size is derived NOT_APPLICABLE |
| OD-78 | *"Like FAILED"* — chosen 2026-09-23, spec-verified: §6.1 rule 2 covers DONE (never again) and FAILED (again only when its cause class changes) but says nothing about PARTIAL. Measured on staging: 238 of 274 LISTED IPOs have never been walked, and while the #884 config gap lasts every walked IPO ends PARTIAL (four anchor-investor fields name the offer document with no document type), so re-picking PARTIAL nightly would give the same newest ten IPOs every slot forever. **A PARTIAL IPO is picked again only when its cause class changes or the source RANKINGS change (the rank lists and capable flags — not any manifest edit); never-walked IPOs take the nightly slots first.** Same reasoning as §6.2: same cause, same outcome, no retry. Clarified by OD-82: the rankings include the document type a `DOC` rank reads. | 2026-09-23 | §6.1, §6.2 | §6.1 rule 2 names PARTIAL; the version that re-opens PARTIAL/FAILED rows is defined as the ranks-and-capability fingerprint |
| OD-79 | *"This recommendation is based on requirements from spec, then yes"* — 2026-09-23, after the third review of the item 17 rebuild proved an IPO recorded `DONE` with 37 plan rows still open (the walk asked 5 fields: 3 answered, 2 not yet available; 35 more rows open). Spec basis: OD-19 ("do not repeat which are already done" — DONE means finished, never looked at again), §6.2 (`cause_class` is "the class of what stopped it" for PARTIAL — PARTIAL means stopped before finishing), OD-76 (nothing asked → DONE only when every row is settled), OD-73 (what "settled" means). **The closed-IPO job records `DONE` only when EVERY plan row of the IPO is settled (OD-73), whatever the walk asked; otherwise `PARTIAL`, re-picked per OD-78.** | 2026-09-23 | §6.1, §6.2 | §6.1 states the every-row-settled DONE rule for every walk, not only a walk that asked nothing |
...
| OD-123 | *"NOT_SOURCED now (Rec.)"* -- 2026-09-26, chosen over (B) find the real printed totals from each prospectus first and (C) leave the rows flagged (Spec basis: manifest ranks for `ipos.issue_size` are DOC then CHITTORGARH only; OD-74 one-time repair from the Chittorgarh detail page; OD-62 "a field no source could supply stores a REASON CODE"; OD-77 `NOT_SOURCED`; §9 admin editing). **An IPO whose issue size is held only from a source the manifest does not rank for it (BSE, NSE, MONEYCONTROL), and for which neither a stored offer document nor a Chittorgarh detail page exists, stores `NOT_SOURCED` instead of that value; the page shows the field as not available.** Measured 2026-09-26 on staging: 7 rows (piyush-ltd Rs 0.70 cr, nirbhay-colours-india-ltd Rs 1.48 cr, induss-food-products-and-equipments-ltd Rs 12.08 cr, banganga-paper-industries-ltd Rs 13.30 cr, sanmitra-commercial-ltd Rs 22.46 cr, morganite-crucible-india-ltd Rs 218.00 cr, kwality-walls-india-ltd Rs 1,303.04 cr). Applied by the same productized, dry-run-first repair tool as OD-74/OD-77; staging now, production with the release. A real printed total can be restored later only through the §9 admin editor, with its source. Closes #979. | 2026-09-26 | §2.5.4, §6.2 | the nightly check `c_issue_size_noncapable_source` reads 0 after the repair |
| OD-124 | *"Show gain % on card (Rec.)"* -- 2026-09-26, chosen over (B) a separate listed-IPO card and (C) do not build (Spec basis: none on card layout -- searched: listing gain, card, recently listed; the value is already sourced in `listing_performance.listing_gain_percent`, 84 of 97 LISTED mainboard IPOs on staging). **A LISTED IPO shown on a card shows its listing gain as a signed, colour-coded percent; when the value is missing it shows nothing, never 0%.** Real rows: Karamtara Engineering Rs 254 -> Rs 352 (+38.58%), Rentomojo Rs 404 -> Rs 534.25 (+32.24%), Manika Plastech Rs 43 -> Rs 43.14 (+0.33%). **Wording correction, 2026-09-26 (owner-questions Q4):** the decision itself is unchanged; only its location and done-when were wrong. The rule is met on the LIVE listing pages by `ListingIndexClient` (`web/components/listing/ListingIndexClient.tsx`)'s `listingGainPercent` column -- signed, colour-coded, an em dash when missing -- not by `IPOCardEnhanced`, which renders on no page today (0 hits for any route importing it). #58 was closed as already met. | 2026-09-26 | §2.11 (read side) | `ListingIndexClient`'s existing gain-percent rendering on `/mainboard-ipos` and `/sme-ipos`, signed/colour-coded/em-dash-when-missing |
| OD-125 | *"No reviews; retire (Rec.)"* -- 2026-09-26, chosen over (B) keep the empty pages and decide later and (C) aggregate named brokers' views with links (Spec basis: none -- this spec covers sourced facts, not editorial opinion; searched: review, score, recommend. Reason given: a subscribe/avoid call published by an AP can count as research-analyst activity under SEBI rules, and no RA registration is recorded in GLOBAL.md). **IPODhan publishes no opinion reviews of IPOs. The empty reviews pages and routes (mainboard and SME review lists) and the never-invoked review scraper/repository path are retired; the facts pages stay.** The IPO score widget is kept and its scale is reconciled with the live score API (widget expects 0-25, API returns 0-10; measured on staging: hy-tech-engineers-ltd 5, tempsens-instruments 6.2). Anchor investors are facts and stay on the IPO page. Closes #167 when built. | 2026-09-26 | §2.11 (read side) | the retired routes return 404 or redirect and are absent from the sitemap; the score widget renders the API value on its own scale |
| OD-126 | *"owner delegation 2026-09-24: go with the recommendation if it conforms to the spec"* (OD-95) -- 2026-09-26, contract 1 (#394/#343/#73; Spec basis: field 13 `sector` D, DOC then CG, check F1 "non-empty, from the fixed sector list"; the spec names no list. Searched: sector, fixed sector list, F1). **The fixed sector list is Chittorgarh's own industry list: 197 code-to-name pairs from its industry index, stored in `scraper/config/sector-list.json`.** CG is field 13's rank-2 source and its detail pages carry exactly one industry code (F-179); all 5 sector values stored on staging before this decision are in the list. A sector not in the list is not written; absent stays NULL, never an empty string. | 2026-09-26 | field 13, §1 | the walk writes `ipos.sector` only from the list; `j_sector_populated` counts empty strings as empty |
| OD-127 | *"owner delegation 2026-09-24: go with the recommendation if it conforms to the spec"* (OD-95) -- 2026-09-26, #932 + #777 (Spec basis: OD-81 event (1) "its stage changed since the last attempt (CLOSED to LISTED)"; OD-56 "read once per STAGE CHANGE"; §6.1 rule 2; §2 "never on a backoff timer"). **OD-81 event (1) is decided from a RECORDED status, not inferred: every closed-IPO job attempt stores the IPO's status as the job selected it (`closed_ipo_resourcing.status_at_attempt`, migration 0063, nullable), and a PARTIAL / FIELDS_PENDING IPO is re-picked on a forward stage change (CLOSED -> LISTED), per OD-81 event (1): recorded CLOSED, LISTED now; a backward flip is not a stage change. A row last attempted before the column existed (NULL) keeps the earlier inference once -- LISTED now and `listing_date` on or after the IST date of the last attempt -- and that attempt records the status.** Why: the inference (#919) missed a status that turned LISTED more than a day after `listing_date` while the job attempted the IPO in the lag, and a LISTED IPO with no `listing_date`. Measured read-only on ipodhan_staging 2026-09-26: LISTED IPOs with NULL `listing_date` 0 of 275; `closed_ipo_resourcing` PARTIAL 59, DONE 1 (all 60 will read NULL after the migration); how often a late status flip happens is unmeasured, because no status history existed. | 2026-09-26 | §6.1 | §6.1 rule 2 names the recorded status; scraper/tests/unit/scheduler/closed-ipo-job.test.ts and the OD-81 case of scraper/tests/integration/closed-ipo-job-plan-then-walk.integration.test.ts |
| OD-128 | Decided in the goal, under the contract delegation ("decided in the goal", spec-conformant) -- 2026-09-26, #648 (Spec basis: §2.10's child-table provenance gap -- `financial_statements`, `promoters`, `ipo_intermediaries`, `peer_companies`, `anchor_investors` written with no consolidated `field_sources` provenance path; a retry was rejected because it fails the same way, on both marker-write failure branches). **A failed provenance-marker (sentinel) write on a child-row-unresolved mark is logged as a named `provenance-marker-write-failed` event carrying its cause (message + code), counted per cycle, and read nightly by the floor check `m_provenance_marker_write_failed`. The residual limit this leaves (a failed mark reads as "writer not live", indistinguishable from the flag being off) is documented in the row-key coverage check's own header.** Done-when: PR #1168 (`child-row-unresolved-noter.ts`'s `createChildRowNoter` factory, both `filing-persister.ts` and `anchor-persister.ts` callers; `scripts/lib/scraper-wake-detection.mjs`'s `checkProvenanceMarkerWriteFailed`). | 2026-09-26 | §2.10 | `scraper/tests/unit/services/child-row-unresolved-noter.test.ts` red-first on a rejecting `trackFieldUpdate`, green after; `m_provenance_marker_write_failed` registered in `scripts/audit-detection-floor.mjs` and `docs/reviews/detection-checks/m_provenance_marker_write_failed.json` |
...
| 13 | `sector` | 196 | D | DOC | CG | MC | keep | F1 | non-empty, from the fixed sector list (OD-126: CG's 197-name industry list, F-179) | CG. | — |
...
**F-185 (2026-09-26):** 0 of 4 real RHP/DRHP documents (A-One Steels RHP and DRHP, German Green RHP,
Green Asia RHP; PR #1161) print a Highest/Lowest peer summary line, so a document-level summary
cross-check never fires on real documents; peer figures are only ever read row-by-row from the peer
table (fields 121-130 above). **F-186:** document peer-table figures are never persisted anyway — the
...
| 164–166 | `gmp_records.timestamp / gmp / source` | 8,369 | **W** | IG | CG | — | ₹/share keep | `|gmp| ≤ 3 × cap`; newest wins | the two grey-market sources against each other. No document, ever. **Job-owned (OD-100, #1022): the GMP job (§2.1) writes this table every 30 minutes; the field-plan walk never asks for it and `gmp_records.gmp` carries no manifest row or plan row at all.** |
...
| **BUYBACK / TENDER** | 17 | Corporate actions, not offerings. **Issue size is `NOT_APPLICABLE` for them, derived from `offering_type` and never stored (§2.7); a stored 0 is removed (OD-77).** They should arguably not be on an IPO site at all (the Mopshop / Sarda class). Field 24 `offering_type` is the guard; §4.6 gives it a check. |
...
| **Opening-day check** | about **09:45**, only on a day an IPO is due to open (OD-31) | `heavy`, skipped if held | the two exchange lists only: register a new or changed IPO so the live jobs can see it | never fetch, download or extract a filing; it writes identity, status and the open and close dates, nothing else (OD-87); a new row is created only from the NSE list, which states the segment, and an IPO on the BSE list only waits for the data job (OD-88); "a day an IPO is due to open" is read from the two fetched lists or the stored rows, so an IPO not yet stored is still caught |
...
**Measured 2026-09-23 on staging (open for owner decision):** **F-142** — after item 7 slice S2 (#935) the 22:15 IST non-slot data wake skipped discovery and the document cycle, but the website list scrapers (`--source=all`) still ran: "Chittorgarh listing report fetched" 12 times and 8 new IPOs created. This row's "three runs a day" is therefore not yet what staging does; slice S2b fixes it. **F-143** — `gmp_records` has no write-time column (its `timestamp` is the source's own as-of time), so the grey-market row's writes cannot be counted by when they happened. **F-151 (2026-09-24, staging):** the 00:00 slot stayed "incomplete" for 16 wakes and never closed, because the document retry timer (30/60 minutes, document-state-machine.ts:465-477) makes the same open documents due on every wake and the 60-second discovery budget runs out each time — the timed retry this section already removes; F-152 records the same breach in the field walk; the data job therefore ran every 30 minutes all night instead of once per slot (#943).
...
| **Image-only pages go to OCR, marked** | **already built.** `scraper/scripts/ocr_pages.py` exists and `extract_filing.py`'s `run()` already takes an `ocr_confidence` map keyed by page (`scraper/scripts/extract_filing.py:2590`). What this design ADDS is one rule on top of it: a field whose only source is an OCR page never wins a disagreement against a text page. **Built (OD-97, F-167):** each value carries its mark (`TEXT` / `OCR` / `MIXED`) in `document_field_receipts.source_text` and in provenance; an OCR-only value is not written over a stored value that a text-page read of a same-or-better-ranked document of the same IPO produced; values from before the mark are unknown and the rule leaves them alone | the OCR path is exercised today; the fixture is captured by `probes/extract-real-pdf.mjs` on a real filing |
...
**Structural refusals are recorded, not re-queued (OD-99).** The PENDING re-queue above is for a
TRANSIENT drop only (a lost lock, an error mid-write). When the writer refuses for a structural
reason it will give on every ask (`MISSING_ROW_KEY`, `CHILD_TABLE_CONSOLIDATION_DISABLED`,
`CONSOLIDATION_DISABLED`), re-queuing loops the row forever: it is claimed first on every wake and no
state names the cause (staging 2026-09-25: 26 `gmp_records.gmp` rows). Such a refusal settles the
row `CHECK_FAILED`, reason code `COVERAGE_GAP`, gap code `WRITER_CANNOT_ACCEPT`, the skip reason in
`cause`, and it is not charged as an attempt (#923). It is asked again only when its gap key
changes; that key carries the writer's capability for the table, so a writer fix reopens it (OD-78,
#884).
...
**Corrected 2026-09-23 (F-136, F-137): the equity ISIN exists before the IPO** (the company's shares
are dematerialised before the draft is filed — NSE's tripartite depository agreements date from
2007/2016), **but our sources did not carry it for open or upcoming IPOs**: the offer documents we
read do not print it for mainboard issues, and neither exchange API returned it. NSDL's free public
search returns it by name before listing (Himalayan Solar INE1B7I01014 and Himalaya Nutravedics
INE1OTR01013, both before listing), and NSE's offer-documents feed and SME RHPs carry it from the
draft stage. And the duplicate proves the point about our own data: the second Asset Reconstruction
row carries **no identifier of any kind**
...
3. **Converging identifiers mean a merge, not an alert.** Two rows sharing one symbol, one CIN or
   ...
   current state. Qualified by OD-71 and OD-83 (2026-09-23): a shared CIN or ISIN proves the same
   COMPANY or share class, not the same offering — a withdrawn or lapsed draft and its later
   refiling share both (NSE: SEBI filing 33921 of 30 Dec 2016 and DRHP 102189 of 18 Jun 2026; Hero
   Motors: DRHP withdrawn 05-10-2024 and DRHP of 30 Jun 2025, F-130), so rows sharing them merge
   only when they are the same attempt; an attempt is defined by filing status before open dates
   exist (OD-84).
...
| 2 | **Each source's own record number (OD-85)** | the day a source first lists the issue | replaces the SEBI draft filing number, which does not exist in that form (F-125); dropped by OD-89 |
...
number" names an identifier that does not exist in that form. **Resolved by OD-89 (2026-09-24): step 2 is
dropped and replaced by the sources' own record numbers (OD-85).**
...
**Matching an incoming record, decided per scenario (OD-68 to OD-71, 2026-09-23).** Every merge run so
far (15 on production, 27 on staging) cleaned up ONE IPO stored twice; no automatic merge has ever
run (the scraper's `iposMerged` counter is a hard-coded 0). The duplicates came from matching, so the
rules sit here, at the match:

| Scenario | Real data | Rule |
|---|---|---|
| S1: a page-status suffix lands in the slug | `rays-of-belief-ltd` (NSE, Rs 74.99 cr) and `rays-of-belief-ltd-o` (NSE+BSE, Rs 125 cr) both live on 2026-09-23; G.V. Electricals, H.R. Hygiene, Shree Balaji (Mala) Textiles were 5 rows each until 2026-08-22 | strip `-o`/`-p`/`-lt`/`-ct` before matching (OD-68) |
| S2: two sources spell the name differently | `ic-electricals-co-ltd` / `-company-ltd`, `dhanwel-hybird` / `-hybrid`, `atharva-poly-plast` / `-polyplast`, ARCIL `co` / `company` | bind by OD-34's identifiers; a name match needs the same price band where both are known and an open date within 180 days (a moved date on an exact-name match is a postponement and updates the row); a same-name, same-segment live row with a differing known date or band that did not bind means hold for review, not a new row (OD-68) |
| S3: a page title stored as the name | `purple-style-labs-ltd-pernia-s-pop-up-studio-ipo`, `national-stock-exchange-of-india-ltd-nse-ipo` | strip title text on write; rename with a slug redirect (OD-68) |
| S4: the same company files again | Polymatech (DRHP Oct 2023, withdrawn 2024, refiling for 2026); boAt / Imagine Marketing (DRHP 2022, abandoned, refiled 2025) | OD-35 unchanged: withdrawn or lapsed = new row; returned-then-refiled = same row (OD-71) |
| S5: a listed company's later event | OFS, TENDER, RIGHTS, BUYBACK rows; SME-to-mainboard migrations | own row of its own type; a migration creates no row (OD-70) |
| S6: different companies, similar names | Himalayan Solar / Himalaya Nutravedics; Technocraft Ventures / Technocrats Plasma | never joined on name; the merge tool refuses differing identifiers or open dates (OD-69) |
| S7: one IPO on both exchanges | 100 of 307 IPOs dual-listed; 99 correctly one row, 1 split (Rays of Belief) | each exchange writes into the one bound row (OD-68) |

Our data holds no refile yet (rows exist only since 2025-12-01, none is WITHDRAWN, and only 41 of 174
filing documents carry a filing date), so S4's tests use the named public cases as fixtures.

**An exchange postponement is the same IPO, not a new one (OD-83).** OD-68 already updates the same
row when an exchange moves an IPO's dates; OD-83 makes explicit that this includes a postponement the
exchange itself labels "issue postponed" followed by a relaunch under a new record number, once the
company, shares and price band match. "Withdrawn" in OD-71 — the trigger for starting a new row — means
only a SEBI withdrawal of the draft offer document or a lapse under OD-35's twelve-month rule; it never
means an exchange-side postponement. Dhanwel Hybrid Seeds (BSE IPO_NO 7794 postponed in June 2026, then
IPO_NO 7900 in August, same 2,700,000 shares and 95–99 band, one unchanged Chittorgarh id) and IC
Electricals (NSE symbol ICEL postponed in June, relaunched as ICELCO in July, same 48,39,600 shares,
94–99 band and CIN) are both one row, updated, not two.

**Before open dates exist, an attempt is defined by filing status (OD-84).** §2.3.3.1 part 3 says
converging identifiers merge only when rows are the "same attempt" and leaves what an attempt is as an
open question (F-130). OD-84 answers it: one attempt is one filed draft offer document (DRHP) plus
everything that follows it in that filing lineage — UDRHP, RHP, prospectus, addenda, corrigenda. A new
full DRHP starts a new attempt only when the prior one is withdrawn, returned, or lapsed twelve months
after SEBI's observations (OD-35); status comes from NSE's offer-documents feed and SEBI's
processing-status reports (F-139). Once an open date exists, OD-35's 180-day rule takes over as before.
Hero Motors (DRHP withdrawn 05-10-2024, a second DRHP received 30-06-2025 that became the 16–18 Sep 2026
IPO) and NSE itself (a 2016 SEBI filing requiring a fresh draft, then a DRHP filed 18-06-2026) are each
...
**Offering type and the other two dates are compared too (OD-94).** Besides OD-69's identifiers and open date, the merge tool refuses a pair whose offering_type values are both populated and differ, relaunch or not: an IPO row and an InvIT row are two offers (OD-70). It also refuses a pair whose close_date or listing_date values are both populated and differ, except under the OD-86 relaunch test, which legitimately moves every date. An empty side is not a disagreement, and the refusal names the column and both values. The staging pair cube-highways-trust (IPO) and cube-highways-trust-cube-highways-trust-invit (INVITS) is the named case (#679).
...
currently-open and recently-filed IPOs, evidenced in `docs/design/findings.json` as F-124 through
F-135. None of these change OD-34, OD-35, OD-68, OD-69 or OD-71 by themselves — each rule change
candidate listed below (F-134) needs its own owner decision recorded as an OD row before it lands
in the binding order or the matching rules.

**(a) What each identifier actually carries, on the same real IPO (NSE IPO itself), F-1 to F-3:**

| identifier | which sources carry it | NSE IPO value |
|---|---|---|
| CIN | offer documents only (DRHP/RHP cover pp. 1, 3); never NSE API, BSE API or Chittorgarh | `U67120MH1992PLC069769` |
| SEBI draft filing number | SEBI per-document only, one number per DRHP/RHP/Prospectus, none linking a draft to its RHP | DRHP 102189 (18 Jun 2026), RHP 104428 (11 Sep 2026), Prospectus 104637 (22 Sep 2026) |
| exchange symbol / record number | NSE symbol (NSE+BSE APIs); BSE IPO_NO + ScripCode (issue-stage, discarded by our scraper); BSE listing code (Chittorgarh pre-listing only); Chittorgarh page id; ISIN (Chittorgarh, a day before listing) | symbol `NSE`; BSE IPO_NO 7977, ScripCode 4827; Chittorgarh page id 3151 |

**Findings, cited by F-id:**

- **F-124** — websites never carry the CIN; only offer documents print it, and only the DRHP feeds
  it today even though the price band ad and RHP also print it.
- **F-125** — SEBI has no single "draft filing number"; it numbers each document separately (see the
  note on OD-34 step 2 above).
- **F-126** — each source has its own record number (NSE symbol, BSE IPO_NO/ScripCode/listing code,
  Chittorgarh page id, ISIN), and we store almost none of them; the BSE scraper fetches by IPO_NO
  and discards it (`bse-api-scraper.ts:449`).
- **F-127** — source record numbers change when an issue is postponed and relaunched (Dhanwel BSE
  IPO_NO 7794→7900; IC Electricals NSE symbol ICEL→ICELCO), while Chittorgarh kept one id.
- **F-128** — the CIN itself changes before an IPO (PTC→PLC), sometimes with its industry code
  (Himalayan Solar, Himalaya Nutravedics, Vinod Texworld); the stable part is state+year+registration
  number.
- **F-129** — offer documents print other companies' CINs (subsidiaries, group companies, the lead
  manager's own CIN); only the cover-page CIN is the issuer's, and current extraction already takes
  the right one for NSE IPO.
- **F-130** — a draft has no open date, so OD-35's 180-day window cannot be computed before an IPO
  opens (Hero Motors, boAt/Imagine Marketing, NSE IPO's own older 2016 filing, Polymatech).
- **F-131** — OD-71 and OD-35 conflict on an exchange postponement + relaunch: if Dhanwel's June
  attempt reads as WITHDRAWN, OD-71 would refuse the August record as a new row while OD-35 (same
  CIN, dates 57 days apart) calls it the same offering. Needs an owner decision.
- **F-132** — name spellings of one IPO vary widely across sources, and some sources embed status
  text inside the name field itself.
- **F-133** — BSE-specific field quirks: a trailing space on Symbol, ScripCode being an issue-stage
  code rather than the listing code, Issue_Size_No_of_shares excluding the anchor portion, and a
  null listing code pre-listing.
- **F-134** — the three-part identity logic (CIN for company; CIN+type+180-day window for offering;
  each source's own number re-binds its record) tested 2 PASS / 4 FAIL on real IPOs. Candidate rule
  changes are listed in the finding and are NOT decided: postponement is not withdrawal; flag rows
  sharing a CIN for merge; match ISIN then trimmed symbol before names; a draft-stage rule; key the
  company on the CIN's stable part; strip status text from names. Each would need its own OD row.
...
**F-184 (2026-09-26, open for owner decision, owner Q5):** real delisted stocks answer NSE 200
"Permanent Suspended" / BSE "Delisted", never no-such-symbol; the no-such-symbol 404 is byte-identical
for a wrong series, a never-existed symbol and a renamed live company (ADANITRANS→ADANIENSOL).
...
| **An `unmerge` command** | `scraper/scripts/repair-merge-duplicate-ipo.ts` gains `--unmerge <merge-id>`: it restores both rows from the log and **re-points the slug redirect** so the page that was redirected goes back to its own row (path corrected by OD-92) |
...
OD-92 makes the log hold "the rows themselves" for everything a merge removes, not only the two IPO rows: every deleted child row and every row a foreign-key cascade removes below it, whole; every reference a SET NULL cascade clears; the prior state of source keys an OD-86 relaunch merge supersedes. `--unmerge` restores all of it in one transaction, parent rows before child rows. A merge logged before OD-92 is partly reversible: the tool lists what the log lacks and restores what it holds only with `--partial`. On the survivor it restores only the columns the merge carried and their provenance; newer values in other columns stay. It refuses when a carried column changed after the merge unless named in `--force-fields`, and it refuses, writing nothing, when a restored row would clash with an existing row under a unique key, naming each clash (inferred rules, recorded in OD-92).
...
**A settled field under an override (OD-95, #968).** An override that ranks a source above the one that settled a field reopens that field once, at the next cycle, and only when the override actually changed the order in a way that outranks the settling source. The reopened row records the override (`reopened_under_policy`), and the walk then asks only the sources above the settling source, so a lower-ranked source cannot overwrite the value (OD-73). If no higher source has the field, or the override ends first, the row returns to its previous settled value; a value a higher source supplied under the override stays after the override expires. A re-plan with no override reopens no settled row. A row is reopened at most once per distinct override (it records the order it was tried under); a higher source that loses to the priority matrix ends the reopen and is listed for the admin (OVERRIDE_SOURCE_LOST_TO_PRIORITY); and a better document that completed during the reopen takes the row through supersession (OD-91) instead of the old evidence coming back.
...
**OD-91 (2026-09-24, F-164):** "the newer document" is decided by each document's own record of the fields it produced (`document_field_receipts`, written at extraction). A row reopens only when the outranking document's record has the field, and the DOC fetcher credits the best-ranked document whose record has it. Documents extracted before the record existed are not re-read (OD-65), so their rows stay as they are.

**OD-96 (2026-09-25, #993):** a document writes a field only when that field's manifest document family contains the document's type — the same family the DOC fetcher credits with — so a value is never written from a document the fetcher will refuse; the stored value stays (OD-7). Applied to `ipos`; child tables open for owner decision (W-76, W-88).
...
**Amended by OD-90 (2026-09-24):** a corrigendum is applied only after admin review. The pipeline turns it into suggestions (field, stored value, proposed value, quoted sentence, page); an admin accepts each as an ADMIN value or dismisses it.

Measured 2026-09-24 (F-158): no extractor reads a corrigendum today. Measured the same day (F-163): of three real corrigenda (Rays of Belief, Skyways, Hy-Tech), only one changes a field a document may write; open for owner decision. CORRIGENDUM is absent from the one extractable-type list, so a stored corrigendum is admitted as NOT_EXTRACTABLE and this subsection has no input yet.

**Follow-up (OD-98, 2026-09-25):** #989 put CORRIGENDUM on the extractable list, but a corrigendum stranded NOT_EXTRACTABLE *before* that fix is never re-admitted by anything, and the document cycle's discovery/extraction candidate set is live-window only — a LISTED IPO past `LIVE_WINDOW_DAYS_AFTER_LISTING` (10 days) is never revisited, so re-admitting the row to PENDING alone would leave it PENDING forever (measured: skyways-air-services-ltd, listed 2026-09-01, 24 days before this decision). OD-98 widens PASS 2 (extraction) only — never discovery — to also consider any IPO holding a stored, never-read, extractable document (PENDING, `purged_unread`=false), regardless of live window.
...
- **Where the words come from:** `chosen_source`, `chosen_document_type` and the read date
  `chosen_confirmed_at` on the plan row (§2.3), stamped when the row is recorded SUPPLIED with its
  evidence (OD-93; `updated_at` is not that date, because any write to the row moves it; rows
  already SUPPLIED with a source were backfilled from `last_attempt_at`, the same write's instant). Read by
  `IpoFieldPlanRepository.getIPOProvenanceMap` (`web/lib/repositories/ipo-field-plan-repository.ts`) —
  one query per page, not one per field. A row supplied before the read date was stored names its
  source with no date, and a block shows a date only when every field in it has one (OD-72: no
  fabricated date). The words are *"From the offer document, read 21 Sep 2026"*, in IST.
- **A live figure shows the time of the figure (OD-72):** *"Subscription as at 10:30 PM, 22 Sep"*,
  from the source's own observation time (`subscriptions.timestamp`), in IST.
- **A missed live slot is an admin alert, not a public label (OD-72, OD-93):** a bidding IPO with
  no subscription figure dated inside one of the 17 half-hour live slots (10:00-18:30 IST, weekdays
  that are not market holidays) is named, with its first missed slot, in one admin alert per IPO per
  IST day, and every missed slot appears in one end-of-day summary; every wake judges every ended slot
  of the day, so a skipped wake is still judged. F-165 measured the
  need on staging: 9 of 19 bidding IPOs had no figure inside the 14:00 IST slot on 2026-09-24.
...
- **The cache key it lives under, and the one it must NOT use:** the detail page's real key is
  ...
  `CacheTTL.IPO_DETAIL = 900` seconds (`web/lib/cache/cache-keys.ts:16`). **Wording fixed by OD-93:**
  the provenance map is a second cached read beside the page payload, under its own slug-keyed key
  `getIPOProvenanceKey(slug)` = `ipo:fieldplan:provenance:<slug>`, and the OD-40 call drops it
  together with `getIPOBySlugKey`; an earlier draft said the line was part of the page payload, which
  the built page does not do.
...
- **The touched slugs survive a restart (OD-93).** They are mirrored into one Redis set; a crash
  between a write and the end-of-cycle call leaves them there, and the next cycle's one call sends
  them with its own.
...
| the SAME source changing its own earlier value | ignored for the page; admin conflicts list under its own named reason, never an alert (OD-75) | none in 30 days |
...
Re-measured 2026-09-26 (pending inventory): sector is empty on 369 of 390 IPO rows though field 13 names DOC then CG (F-175); 8,421 field-plan rows sit in CHECK_FAILED, re-claimed but unexplained (F-177). Contract 1 (2026-09-26): Chittorgarh detail pages carry one numeric industry code that its industry index names (F-179, OD-126); the lead-manager provenance upsert failed with 42P10 on every call while its log dropped the database cause (F-180); same-source unresolved conflicts are still created daily from four code defects (F-181, bears on OD-75 and #818); the nightly floor audits production, which lags main by many migrations, so a check reading a new column must probe for it (F-182).
...
Measured 2026-09-24 (F-156): the unit tags do not yet follow this. `field-manifest.json` tags these five rupee-stored columns `crore`, and the admin issue-size label and validation assume crore, although both are keyed to `issueSize` while the admin screen that writes the column reads `issue_size`, so they are dead code rather than a live error (F-156 correction); item 11 makes the tags follow the unit stored today.
...
2. Not already marked done, and not marked failed **or partial** with the **same cause class** as last time, unless the source rankings changed since (OD-78). A `PARTIAL` IPO whose cause is `FIELDS_PENDING` (OD-80) is picked again only on an event since its last attempt — its stage changed, a new document for it was first seen, or the source rankings changed (OD-81) — never on a timer. Never-walked IPOs come first.
3. Ordered by `close_date` **descending** — newest closed first, which is the owner's sequence.
4. **At most ten.**

**What it does to each one (OD-76):** if the IPO has no field plan yet, it first runs the existing plan generator for that IPO; then exactly the pull walk of §2.4 over that IPO's field plan. An IPO whose plan could not be generated (or has no rows) is never recorded `DONE`; an IPO is `DONE` only when every one of its plan rows is settled (OD-73), whatever the walk asked (OD-79); otherwise `PARTIAL`. Otherwise:, with the
...
| `cause_class` | for `PARTIAL`/`FAILED`: the class of what stopped it — `DOCUMENT_UNOBTAINABLE`, `EXTRACTOR_MISSING`, `VALIDATION_REJECTED`, `SOURCE_UNREACHABLE`, `WRITE_SKIPPED`, `FIELDS_PENDING` (OD-80: fields remain not due yet or waiting to retry) |
...
|---|---|---|---|---|---|
| 1 | **The child-table consolidated writer** — **PREREQUISITE FOUND 2026-09-09 (F-74): `normalized_name` does not exist on `promoters`, `peer_companies` or `ipo_intermediaries`** (`probes/duplicate-scan.mjs`, read against production; verified against `packages/shared/src/db/schema.ts`). The card's UNIQUE constraints name a column that has never been written, so the migration cannot run as specified: the column is added and backfilled first, THEN the duplicate scan is re-run with the real normaliser, THEN the constraint is added. Under a stand-in lower-and-trim normalisation there are zero duplicate groups today, which is encouraging and is not the same test — — extend the consolidation contract to `ipo_details`, `financial_statements`, `ipo_valuation`, `ipo_risk_factors`, `promoters`, `anchor_investors`, `ipo_intermediaries`, `peer_companies`: per-field priority resolution, `field_sources` rows, `data_conflicts` rows | — | **A** | **large — and it is the gate on everything below** | `consolidation` |
