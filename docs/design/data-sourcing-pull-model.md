...
| S2: two sources spell the name differently | `ic-electricals-co-ltd` / `-company-ltd`, `dhanwel-hybird` / `-hybrid`, `atharva-poly-plast` / `-polyplast`, ARCIL `co` / `company` | bind by OD-34's identifiers; a name match needs the same open date AND price band; otherwise hold for review (OD-68) |
