...
| OD-99 | *"owner delegation 2026-09-24: go with the recommendation if it conforms to the spec"* — 2026-09-25 (structural write refusals; Spec basis: OD-62 a reason code, never a silent null; OD-78 same cause, same outcome, no retry; OD-73 "An identical incoming value is never written and never re-stamps provenance" and class W live figures follow newest; section 2.1 the GMP job writes `gmp_records` every 30 min; section 1 rows 164-166 `gmp_records.gmp` class W, INVESTORGAIN_GMP rank 1; #884 gap codes, re-asked when the key changes). **A write the consolidated writer refuses for a STRUCTURAL reason is recorded, never silently re-queued, and an answer equal to the stored value is recorded without a write.** (a) A structural refusal (`MISSING_ROW_KEY`, `CHILD_TABLE_CONSOLIDATION_DISABLED`, `CONSOLIDATION_DISABLED`: the writer will refuse the same way every time it is asked) settles the plan row `CHECK_FAILED` with reason code `COVERAGE_GAP`, gap code `WRITER_CANNOT_ACCEPT`, the attempt counted, `last_attempt_at` set and the skip reason in `cause`. It is asked again only when its gap key changes, and that key carries the writer's capability for the table (the consolidation flags, whether the table is keyable as one row per IPO, a writer version bumped by a code fix), so a writer fix reopens it. A transient drop (a lost lock, an error mid-write) keeps the PENDING re-queue of section 2.3. (b) When the answering source's value equals the value already stored for that field in its target table, compared exactly as numbers (`1250000000` equals `1250000000.00`), the plan row is recorded `SUPPLIED` with that source and nothing is written. The fetcher declares the stored value it read; the INVESTORGAIN_GMP fetcher does, because it answers from the `gmp_records` row the GMP job already wrote. (b) is decided before the write, so it takes precedence over (a). Measured before the fix on staging 2026-09-25: 26 `gmp_records.gmp` rows looped on every wake as `MISSING_ROW_KEY` (failure class `dropped-write-strands-a-plan-row-pending`). | 2026-09-25 | section 2.3 | section 2.3's write-result paragraph states the structural refusal, the writer gap key, the equal-value rule and the transient re-queue, and cites OD-99 |
...
row `CHECK_FAILED`, reason code `COVERAGE_GAP`, gap code `WRITER_CANNOT_ACCEPT`, the attempt counted
and the skip reason in `cause`. It is asked again only when its gap key changes; that key carries
the writer's capability for the table, so a writer fix reopens it (OD-78, #884). **An answer equal to
the stored value is not written (OD-73, OD-99):** when the fetcher declares the value already stored
for the field in its target table and the answer equals it (exactly, as numbers), the row is
recorded `SUPPLIED` from that source and the writer is not called. This is checked before the write.
