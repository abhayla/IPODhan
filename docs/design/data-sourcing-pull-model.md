...
- **Where the words come from:** `chosen_source`, `chosen_document_type` and the read date
  ...
  evidence (OD-93; `updated_at` is not that date, because any write to the row moves it). Read by
...
- **A missed live slot is an admin alert, not a public label (OD-72, OD-93):** a bidding IPO with
  ...
  that are not market holidays) is named, with the slot, in one admin alert per slot. F-165 measured the
