...
**What it does to each one (OD-76):** if the IPO has no field plan yet, it first runs the existing plan generator for that IPO; then exactly the pull walk of §2.4 over that IPO's field plan. An IPO whose plan could not be generated (or has no rows) is never recorded `DONE`; one whose walk asked nothing is `DONE` only when every plan row is already settled (OD-73). Otherwise:, with the
