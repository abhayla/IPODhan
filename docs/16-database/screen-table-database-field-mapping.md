...
**Schema Version:** 0029
**Documentation Version:** 2.0 (Monolithic - Legacy)
**Part:** Original (kept for backward compatibility)

---

**⚠️ Note:** This is the original monolithic mapping document. It has been split into 7 focused documents for better maintainability. See [Master Index](screen-database-mapping-index.md) for the new structure.
...
**Database Schema Source**: `packages/shared/src/db/schema.ts` (canonical source of truth)
**Re-exported via**: `web/lib/db/index.ts` (for application imports)
**Auto-generated migration schema**: `web/drizzle/migrations/schema.ts` (DO NOT EDIT - generated by Drizzle Kit)

**⚠️ IMPORTANT - Field Name Updates:**
This document uses legacy field names. The actual schema uses:
- `priceRangeMin` / `priceRangeMax` (not `price_band_low/high`)
- `gmpPrice` (not `gmp`)
- `gmpPercentageHistorical` (not `gmp_percentage`)
- `segment` (enum: MAINBOARD/SME, nullable) + `offeringType` (enum: IPO/FPO/RIGHTS/etc.) - replaced single `category` field (Migration 0015)

See split mapping documents for updated field names.
...
##### 9. **GMP Confidence Score** ⭐

**Note:** This feature was planned but the `gmp_tracking` table does not exist in the current schema.
The actual schema only has `gmpRecords` table without confidence scoring fields.

**Proposed Database Fields** (if implemented):
- `confidenceScore` (INTEGER, 1-100) - Reliability score
- `source` (VARCHAR) - Data source
- `sourceUrl` (VARCHAR) - Source verification link

**Impact:** No indication of GMP data quality/reliability to users.

**Recommendation:**
- Add confidence scoring fields to existing `gmpRecords` table
...
##### 12. **Anchor Investors (`anchorInvestors` table) - COMPLETELY UNMAPPED** ⭐⭐

**Database Fields:**
- `ipoId` (UUID, FK) - IPO reference
- `bidDate` (DATE) - Anchor bidding date
- `totalSharesOffered` (BIGINT) - Total anchor allocation
- `totalAmountRaised` (NUMERIC) - Amount in crores
- `anchorInvestorsCount` (INTEGER) - Number of anchor investors
- `lockIn50PercentDate` (DATE) - 50% lock-in expiry
- `lockInRemainingDate` (DATE) - Remaining lock-in expiry
- `investorList` (JSONB) - Array of {name, type, shares, amount, percentOfIssue}

**Impact:** Critical institutional investment data completely hidden from users. Anchor investor participation is a strong signal of IPO quality.

**Recommendation:**
- Add "Anchor Investors" tab to IPO detail page
- Display investor list with allocation breakdown
- Show total anchor allocation statistics
- Highlight lock-in period timelines

---

##### 13. **Manual Data Management (`fieldProtectionMetadata` table) - ADMIN FEATURE** 🔧

**Database Fields:**
- `tableName` (VARCHAR) - Protected table name
- `fieldName` (VARCHAR) - Protected field name
- `ipoId` (UUID, FK) - IPO reference
- `isProtected` (BOOLEAN) - Protection flag
- `autoProtected` (BOOLEAN) - Auto-lock after manual edit
- `manuallyEditedAt` (TIMESTAMP) - Edit timestamp
- `manuallyEditedBy` (VARCHAR) - Admin username
- `editNote` (TEXT) - Reason for manual edit

**Impact:** Admin-only feature for preventing scrapers from overwriting manually corrected data.

**Status:** Backend implementation exists (Phase 6), not user-facing. Correctly unmapped from UI.

---

##### 14. **System Configuration (`adminSettings` table) - ADMIN FEATURE** 🔧

**Database Fields:**
- `settingKey` (VARCHAR, unique) - Configuration key
- `settingValue` (TEXT) - Configuration value (JSON or text)

**Impact:** Application configuration management.

**Status:** Admin-only feature. Correctly unmapped from UI.

---

##### 15. **Audit Trail (`auditLogs` table) - ADMIN FEATURE** 🔧

**Database Fields:**
- `timestamp` (TIMESTAMP) - Action time
- `adminUser` (VARCHAR) - Username
- `actionType` (VARCHAR) - Action performed
- `ipoId` (UUID, FK, nullable) - Related IPO
- `tableName` (VARCHAR) - Modified table
- `fieldName` (VARCHAR) - Modified field
- `oldValue` (TEXT) - Previous value
- `newValue` (TEXT) - New value
- `details` (JSONB) - Additional context
- `ipAddress` (VARCHAR) - Admin IP
- `userAgent` (TEXT) - Browser info
- `success` (BOOLEAN) - Success flag
- `errorMessage` (TEXT) - Error details

**Impact:** Immutable audit trail for compliance and debugging.
...
##### GMP Data:
1. **`gmpRecords`** - Currently mapped, used in UI ✅ ONLY TABLE THAT EXISTS

**⚠️ CORRECTION:** Earlier versions of this document incorrectly mentioned `gmp_history` and `gmp_tracking` tables.
These tables do NOT exist in the current schema. Only `gmpRecords` exists.

**Recommendation:**
- Add confidence scoring fields to existing `gmpRecords` table if GMP reliability tracking is needed

---

##### Subscription Data:
1. **`subscriptions`** - Currently mapped, comprehensive ✅ ONLY TABLE THAT EXISTS

**⚠️ CORRECTION:** Earlier versions of this document incorrectly mentioned a `subscription_data` table.
This table does NOT exist in the current schema. Only `subscriptions` exists.

**Recommendation:**
- Continue using `subscriptions` table (no changes needed)
...
**Effort:** Medium | **Impact:** High | **Priority:** Infrastructure

**⚠️ UPDATE:** GMP and subscription "duplicate tables" do NOT exist. Only these tables are real:
- `gmpRecords` (only GMP table)
- `subscriptions` (only subscription table)

**Actual Tasks:**
- Evaluate whether to migrate from `financial_data` to `ipo_financials` (both exist, `ipo_financials` has more fields)
- Document table usage and schema evolution in developer docs
- Add field mapping migration guide for developers
