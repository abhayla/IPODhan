...
/**
 * Mainboard IPO Reviews & Analysis Page
 *
 * Displays expert IPO reviews and analysis from SEBI registered analysts.
 * Features:
  ...
  return (
    <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6">
      <h2 className="text-xl font-semibold mb-3 text-gray-900">
        Mainboard IPO Reviews & Analysis
      </h2>
      <p className="text-sm text-gray-700 mb-2">
        IPO forecast helps investors decide if the IPO is worth investing in.
        Our analysis covers:
      </p>
...
export default function MainboardIPOReviewsPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        const result = await getMainboardIPOReviews(Number(year), {
        ...
        setError('Failed to load IPO reviews. Please try again later.');
        setData([]);
        setTotalCount(0);
      } finally {
        setLoading(false);
  ...
  const handleYearChange = (newYear: string) => {
    setYear(newYear);
    setPage(1);
    router.push(`/mainboard-ipo-reviews?year=${newYear}`);
  };
  ...
  const handlePageChange = (newPage: number) => {
    ...
    router.push(`/mainboard-ipo-reviews?${params.toString()}`);
  };
