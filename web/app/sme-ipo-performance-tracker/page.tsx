...
import { getSMEPerformanceData } from '@/lib/services/mainboard-performance-service';
import {
...
export const metadata: Metadata = {
  title: `SME IPO Performance Tracker ${new Date().getFullYear()} - Post-Listing Analysis | IPODhan`,
  description:
...
export default async function SMEPerformanceTrackerPage({
  ...
  // P3-13 (T-302): server-render the initial rows for the requested year —
  // see the mirrored fix + rationale on the Mainboard tracker page.
  const initialData = await getSMEPerformanceData(year);
  ...
  return (
    <>
      ...
      <Suspense fallback={<PerformanceTrackerSkeleton />}>
        <SMEPerformanceTrackerClient initialYear={year} initialData={initialData} />
      </Suspense>
    </>
  );
}
