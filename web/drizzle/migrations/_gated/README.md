...
Row-key uniqueness, pull-model implementation loop item 1 (owner-applied, ordered apply
required — see the file's own header):

10. `E1_row_key_unique_constraints.sql` — item 1 slice s2: adds
    `UNIQUE (ipo_id, normalized_name)` on `promoters` and `peer_companies`, and
    `UNIQUE (ipo_id, role, normalized_name)` on `ipo_intermediaries` (a 2-column key
    on that table would reject 5 real rows — same legal entity, two distinct roles
    for one IPO), then drops the `''` default on all three `normalized_name`
    columns. **MUST NOT be applied until `scraper/scripts/backfill-normalized-name.ts`
    has populated `normalized_name` on that slot (0 rows still at `''`)** —
    applying it first hits the pre-existing `''` rows on the very first ADD
    CONSTRAINT and dies mid-migration. Generated by `drizzle-kit generate` (clean
    diff, no journal drift for this table set) and hand-moved here; schema.ts keeps
    the `''` default until this file is applied (precedent: entry 1–8 above).

11. `E2_risk_factor_heading_hash_key.sql` — item 1 slice s6: re-keys
    `ipo_risk_factors` from the POSITIONAL `unique_ipo_risk_factors_ipo_seq` to
    `UNIQUE (ipo_id, heading_hash)`, then drops the `''` default on
    `heading_hash`. The column itself ships journaled
    (`20260910121813_cooing_manta`, ADD COLUMN with a `''` default — safe
    unattended on the ~2130 existing rows); only the constraint swap is gated.
    **MUST NOT be applied until `scraper/scripts/repair-risk-factor-heading-hash.ts`
    has run BOTH phases on that slot** — `--backfill --apply` (0 rows still at
    `''`) and `--dedupe --apply` (0 surplus rows). Staging carries 7 groups of
    byte-identical duplicate risk factors (14 surplus rows, measured
    2026-09-10), so applying the constraint first dies on the first
    ADD CONSTRAINT. Apply order and the precheck: `docs/ops/prod-ops-recipes.md`
    §8d.
