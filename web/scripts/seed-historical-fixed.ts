/**
 * Fixed Historical IPO Seed Script
 * Corrected schema field names to match database
 */
 ...
 */
function generateSlug(companyName: string, index: number): string {
...
/**
 * Create IPO record with CORRECT schema fields
 */
async function createIPO(data: HistoricalIPOData, index: number): Promise<boolean> {
  try {
    const slug = generateSlug(data.ipoName, index);
    ...
    // CORRECTED: Use proper schema field names
    const newIPO = {
      ...
      // Pricing - CORRECTED field names
      priceRangeMin: data.issuePrice ? Math.round(data.issuePrice) : null,
      priceRangeMax: data.issuePrice ? Math.round(data.issuePrice) : null,
 ...
 */
async function main() {
  console.log('🌱 Fixed Historical IPO Seeding\n');
  ...
  try {
    ...
    for (let i = 0; i < allIPOs.length; i++) {
      const ipo = allIPOs[i];
      const result = await createIPO(ipo, i);
      if (result) {
        created++;
        if (created <= 10 || created % 50 === 0) {
          console.log(`  ✅ ${created}/${allIPOs.length}: ${ipo.ipoName}`);
        }
      } else {
        skipped++;
      }

      // Stop at 200 to avoid too many records
      if (created >= 200) {
        console.log(`\n  ⚠️  Reached 200 created IPOs, stopping...`);
        break;
      }
    }
    ...
    if (finalCount >= 150) {
      ...
      console.log(`⚠️  Progress: ${finalCount}/150 IPOs\n`);
    }
