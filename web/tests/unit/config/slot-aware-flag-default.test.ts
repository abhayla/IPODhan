/**
 * item 01 slice s5a — web mirror of the scraper's slotAwareFlagDefault() test.
 * Must default ON for the `staging` DEPLOY_SLOT and OFF everywhere else
 * (prod, unset), and an explicit value on the flag's own env var must always
 * win over the slot default, in either direction.
 *
 * Loads the REAL feature-flags module fresh via vi.resetModules() + dynamic
...
describe('slotAwareFlagDefault (item 01 slice s5a, web)', () => {
  afterEach(() => {
