...
## DRHP Extraction (HISTORICAL - removed T-407, 2026-09-02)

The auto-download/auto-extract DRHP pipeline described in earlier revisions
of this doc (SEBI/exchange monitors, drhp-orchestrator, the admin extract
API route) was never wired into production (0 callers, feature flag always
off) and was deleted in T-407 as dead code.

The live document path is described in
`docs/reviews/ipo-document-lifecycle-plan.md`, with current status tracked
in `docs/reviews/T-403-plan.md`.

Two CLI tools remain and are still used for manual/backfill financial
extraction:
- `scraper/scripts/extract_financials_pdf.py`
- `scraper/scripts/backfill-financials-pdf.ts`
