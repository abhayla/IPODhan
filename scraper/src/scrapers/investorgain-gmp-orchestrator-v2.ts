/**
 * Investorgain GMP Orchestrator V2 - With Protection Enforcement
 *
 * Orchestrates GMP data scraping from investorgain.com API with manual data protection
 * Matches GMPs to existing IPOs by dates, creates gmp_records entries
 *
 * **Phase 2: Protection Integration**
 * - Checks IPO-level lock before creating GMP records
 * - Checks field-level protection for gmp_records table
 * - Logs all blocked updates to Redis
 * - Maintains existing matching logic (company name similarity)
 *
 * @see docs/00-admin/MANUAL_DATA_MANAGEMENT_PLAN.md - Phase 2
 * @see scraper/src/scrapers/investorgain-gmp-orchestrator.ts - Original version
 */
...
export interface InvestorgainGMPResult {
  ...
  gmpsBlocked: number; // NEW: Blocked by protection (IPO locked or field protected)
  gmpsFailed: number;
  errors: string[];
}
...
/**
 * Check if GMP creation is protected for an IPO
 * Protection applies if:
 * 1. IPO is locked (scraper_locked = true)
 * 2. gmp_records table fields are protected for this IPO
 *
 * @param ipoId - IPO ID to check
 * @param gmpData - GMP data to be created
 * @returns true if protected, false if allowed
 */
async function isGMPProtected(
  ipoId: string,
  ...
  try {
    ...
    if (locked) {
      logger.debug(
        { ipoId },
        'GMP creation blocked - IPO is locked'
      );
      return true;
    }
    ...
    if (isProtected) {
      logger.debug(
        { ipoId, originalFields: Object.keys(gmpData), filteredFields: Object.keys(filteredData) },
        'GMP creation blocked - fields are protected'
      );
    }
    ...
    logger.error(
      ...
      'Error checking GMP protection - allowing creation to be safe'
    );
    // Fail open: if protection check fails, allow creation
    return false;
  }
}

/**
 * Run Investorgain GMP scraper workflow V2 (with protection)
 * Orchestrates scraping, matching, protection checks, persistence, and cache invalidation
 */
export async function runInvestorgainGMPScraper(): Promise<InvestorgainGMPResult> {
  ...
  const result: InvestorgainGMPResult = {
    ...
    gmpsCreated: 0,
    gmpsSkipped: 0,
    gmpsBlocked: 0, // NEW: Protection tracking
    gmpsFailed: 0,
    errors: []
  };

  try {
    logger.info('Investorgain GMP scraper orchestrator V2 started (with protection checks)');
          ...
          logger.info(
            {
              ...
              ipoId,
              gmp: scrapedGMP.gmp,
              reason: 'IPO locked or GMP fields protected'
            },
            'Skipping GMP creation - protected by manual data management'
          );

          // Note: Blocked updates are logged via filterProtectedFields internally

          continue;
        }

        // Create GMP record (not protected)
    ...
    logger.info(
      ...
      'Investorgain GMP scraper orchestrator V2 completed'
    );
    ...
    logger.error(
      ...
      'Investorgain GMP scraper orchestrator V2 failed'
    );
