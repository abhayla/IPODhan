 ...
 * ruling 33). NSE joined later (#705/#759). INVESTORGAIN_GMP's adapter
 * (`field-plan-walk-investorgain-gmp-fetcher.ts`) was RETIRED under OD-100
 * (#1022): `gmp_records.gmp` is job-owned (the GMP job, section 2.1) and no
 * longer gets a manifest row at all, so the walk never asks for it — see
 * `buildFieldPlanWalkFetchers`'s own doc comment for what an unregistered
 * source means for a plan row ranking it.
 *
 ...
 * the source (`NSE`, `BSE`, `CHITTORGARH`, …).
 *
 * Stage 2 wired DOC, BSE and CHITTORGARH — the three sources every field in
 * that stage's staging plan actually ranked (`ipos.issue_size`,
 * `ipo_details.fresh_issue`/`.ofs_issue`/`.min_investment`,
 * `financial_statements.revenue`); NSE joined later (#705/#759). A field
 * ranking a source with no adapter still answers `NO_FETCHER_REGISTERED`
 * (`field-plan-walk.ts`'s `!fetcher` branch), which is TRANSIENT (F1), not
 * terminal: those rows stay PENDING on backoff until an adapter lands.
 * INVESTORGAIN_GMP had an adapter (stage 2 item 6) and was RETIRED under
 * OD-100 (#1022): `gmp_records.gmp` no longer gets a manifest row at all, so
 * no fetcher is registered for this source key any more.
 *
...
export function buildFieldPlanWalkFetchers(
  ...
  return {
    ...
    // INVESTORGAIN_GMP retired under OD-100 (#1022): gmp_records.gmp is
    // job-owned and carries no manifest row, so no fetcher is registered here.
  };
}
