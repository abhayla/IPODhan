 ...
 * consolidation throws) keeps a stored terminal ipo status — WITHDRAWN, POSTPONED, DELISTED —
 * exactly like the consolidation path's TERMINAL_IPO_STATUSES guard. DELISTED is set and cleared
 * only by the post-listing price job.
 */
...
describe('keepTerminalIpoStatus (the fallback door)', () => {
  for (const stored of ['DELISTED', 'WITHDRAWN', 'POSTPONED']) {
  ...
  it('the same terminal value, or no status key, passes unchanged', () => {
    expect(keepTerminalIpoStatus('DELISTED', { status: 'DELISTED' })).toEqual({ status: 'DELISTED' });
    expect(keepTerminalIpoStatus('DELISTED', { companyName: 'Y' })).toEqual({ companyName: 'Y' });
  });
