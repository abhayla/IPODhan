/**
 * T-492 batch 1 (#386): backfill-lot-size-from-chittorgarh.ts previously had NO prod guard at all
 ...
 * entry point backfill-lot-size-from-chittorgarh.ts's main() calls before any read/write) refuses
...
describe('backfill-lot-size-from-chittorgarh.ts — prod-write refusal via openRepairDb()', () => {
  it('refuses --apply against current_database()="ipodhan" without --allow-prod', async () => {
    ...
    expect(refusedReason).toMatch(/backfill-lot-size-from-chittorgarh/);
