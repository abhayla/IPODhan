...
/**
 * T-493 (#386 batch 2): backfill-registrar-chittorgarh-detail.ts previously had NO shared prod guard — it
...
describe('backfill-registrar-chittorgarh-detail.ts — prod-write refusal via openRepairDb()', () => {
  it('refuses --apply against current_database()="ipodhan" without --allow-prod', async () => {
    ...
    expect(refusedReason).toMatch(/backfill-registrar-chittorgarh-detail/);
