...
class TestIPODataValidator:
    ...
    def test_validate_ipo_data_invalid_dates(self):
        ...
        assert result.is_valid is False
        assert any(
            "close_date must be after open_date" in error for error in result.errors
        )
    ...
    def test_validate_ipo_data_invalid_price_band(self):
        ...
        assert result.is_valid is False
        assert any(
            "price_band_high must be greater" in error for error in result.errors
        )
    ...
    def test_validate_ipo_data_invalid_lot_size(self):
        ...
        assert result.is_valid is False
        assert any(
            "lot_size must be a positive integer" in error for error in result.errors
        )
    ...
    def test_validate_gmp_data_negative_gmp(self):
        ...
        assert result.is_valid is False
        assert any(
            "gmp_amount must be non-negative" in error for error in result.errors
        )
