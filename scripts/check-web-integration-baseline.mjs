...
function toPosix(p) {
  // Unconditional replace, never gated on the host's own `path.sep`: a report
  // generated by a Windows worker (backslash paths) and read on a Linux
  // runner (or vice versa) must normalize regardless of which OS is doing
  // the reading (MINOR-5).
  return p.split('\\').join('/');
}
...
export function extractFailingFiles(report, root = ROOT) {
  ...
  // MAJOR-3: a report with testResults=[] collected ZERO test files (a broken
  // config, a glob matching nothing, or vitest dying before it discovered any
  // suite) - that is a broken run, not a pass. Distinct from a report that
  // collected files where none failed (a genuine pass), which is testResults
  // with entries but none status:'failed'. Fail closed on the former.
  if (report.testResults.length === 0) {
    throw new Error(
      'vitest JSON report collected ZERO test files ("testResults": []) - this is a broken ' +
        'run (bad config, a glob matching nothing, or vitest dying before discovering any ' +
        'suite), not a passing one. A report with test files where none failed is a genuine ' +
        'pass; this is not that.'
    );
  }
...
function main() {
  ...
  // MINOR-6 / signal-ownership R6: every gate prints its reason before a
  // non-zero exit - never let a raw stack trace be the only signal.
  let failingFiles;
  try {
    failingFiles = extractFailingFiles(report);
  } catch (err) {
    console.error(
      `[web-integration-baseline] FAIL — report at ${args.report} is unusable: ${err.message}`
    );
    return 1;
  }
